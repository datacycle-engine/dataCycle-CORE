...
configs.each do |file_name|
  ...
  DataCycleCore.send(File.basename(file_name, '.*') + '=', DataCycleCore.try(File.basename(file_name, '.*'))&.deep_merge(YAML.safe_load(ERB.new(File.read(file_name)).result, [Symbol]))).freeze
end
