...
DataCycleCore::Engine.routes.draw do
  ...
  scope '(/watch_lists/:watch_list_id)', defaults: { watch_list_id: nil } do
    resources(*['organizations', 'persons', 'events', 'places'].map(&:to_sym), only: [:index, :show, :create, :edit, :update, :destroy], controller: :things) do
  ...
  defaults format: :json do
    namespace :api do
      ...
      namespace :v2 do
        ...
        resources(*['organizations', 'persons', 'events', 'places'].map(&:to_sym), only: [:index, :show]) do
        end
