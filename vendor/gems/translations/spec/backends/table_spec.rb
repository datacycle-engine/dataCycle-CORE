...
describe 'Translations::Backends::ActiveRecord::Table', orm: :active_record do
  require 'translations/backends/active_record/table'
  extend Helpers::ActiveRecord

  before do
    stub_const 'Article', Class.new(::ActiveRecord::Base)
    Article.extend Translations
  end
  ...
  context 'without cache' do
    ...
    # include_cache_key_examples 'Article' # does not work for now (not actively used)

    it 'finds translation on every read/write' do
      article = Article.new
      title_backend = article.translation_backends[:title]
  ...
  context 'with cache' do
    ...
    it 'resets model translations cache when model is saved or reloaded' do
      article = Article.new
      title_backend = article.translation_backends[:title]
      content_backend = article.translation_backends[:content]

      aggregate_failures 'cacheing reads' do
        title_backend.read(:en)
        expect(article.instance_variable_get(:@__translation_translations_cache).size).to eq(1)
        content_backend.read(:en)
        expect(article.instance_variable_get(:@__translation_translations_cache).size).to eq(1)
        content_backend.read(:ja)
        expect(article.instance_variable_get(:@__translation_translations_cache).size).to eq(2)
      end

      aggregate_failures 'resetting cache' do
        article.save
        expect(article.instance_variable_get(:@__translation_translations_cache).size).to eq(0)
  ...
  describe 'translations association' do
    ...
    describe 'cleaning up blank translations' do
      let(:title_backend) { article.translation_backends[:title] }

      it 'builds nil translations when reading but does not save them' do
        I18n.locale = :en
        article = Article.new(title: 'New Article')
        association_name = article.translation_backends[:title].association_name
  ...
  describe 'Backend methods' do
    before do
      ...
      ['foo', 'bar', 'baz'].each { |slug| Article.create!(slug: slug) }
    end
    let(:article) { Article.find_by(slug: 'baz') }
    let(:title_backend) { article.translation_backends[:title] }
    let(:content_backend) { article.translation_backends[:content] }
  ...
  describe 'translation scope (.i18n)' do
    before { Article.translates :title, :content, backend: :table, cache: false }
    ...
    describe 'joins' do
      ...
      describe 'Arel queries' do
        ...
        it 'combines multiple locales to use correct join type' do
          ...
          Article.create(title: 'foo en')

          Article.i18n(locale: :en) { |en|
            Article.i18n(locale: :ja) do |ja|
  ...
  describe 'Model.i18n.find_by' do
    ...
    it 'finds correct translation if exists in current locale' do
      ...
      expect(Article.i18n.find_by(title: 'タイトル')).to eq(article)
      expect(Article.i18n.find_by(title: 'foo')).to be_nil
    end
    ...
    it 'returns nil if no matching translation exists in this locale' do
      ...
      I18n.locale = :en
      expect(Article.i18n.find_by(title: 'タイトル')).to eq(nil)
      expect(Article.i18n.find_by(title: 'foo')).to be_nil
    end
    ...
    it 'works on a scope' do
      ...
      I18n.with_locale(:ja) do
        expect(Article.i18n.all.find_by(title: 'タイトル')).to eq(article1)
      end
    end
  end
end
