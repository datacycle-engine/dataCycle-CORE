...
describe Translations::Backend do
  context 'included in backend' do
    ...
    before do
      ...
      backend.class_eval do
        ...
        define_method :write do |locale, _value, options = {}|
          backend_double_.read(locale, options)
        end
      end
    ...
    describe '.each' do
      ...
      it 'yields translations to each_locale if method is defined' do
        ...
        translations = backend.each_with_object([]) do |translation, all_translations|
  ...
  describe '.method_name' do
    it 'returns <attribute>_translations' do
      expect(Translations::Backend.method_name('foo')).to eq('foo_backend')
    end
  end
end
