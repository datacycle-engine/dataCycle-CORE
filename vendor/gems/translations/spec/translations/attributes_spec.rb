...
describe Translations::Attributes do
  ...
  # and delegate read and write to the double.
  ...
  describe 'initializing' do
    ...
    it 'raises BackendRequired error if backend is nil and no default is set' do
      expect { described_class.new('title') }.to raise_error(Translations::BackendRequired)
    end

    it 'does not raise error if backend is nil but default_backend is set' do
      original_default_backend = Translations.config.default_backend
      Translations.config.default_backend = :null
      expect { described_class.new('title') }.not_to raise_error
      Translations.config.default_backend = original_default_backend
    end
  end

  describe 'including Attributes in a model' do
    let(:expected_options) { { foo: 'bar', **Translations.default_options, model_class: model_class } }
    ...
    it 'assigns options to backend class' do
      ...
      expect(attributes.backend_class.options).to eq(Translations.default_options.merge(model_class: model_class, foo: 'bar'))
    end
    ...
    describe 'model class methods' do
      ['translation_attributes', 'translated_attribute_names'].each do |method_name|
      ...
      describe '.translation_attribute?' do
        it 'returns true if and only if attribute name is translated' do
          names = ['title', 'content']
          model_class.include described_class.new(*names, backend: :null)
          names.each do |name|
            expect(model_class.translation_attribute?(name)).to eq(true)
            expect(model_class.translation_attribute?(name.to_sym)).to eq(true)
          end
          expect(model_class.translation_attribute?('foo')).to eq(false)
        end
      end

      describe '.translation_modules' do
        it 'returns attribute modules on class' do
          ...
          expect(model_class.translation_modules).to match_array(modules)
        end
      end

      describe '.translation_backend_class' do
        it 'returns backend class for attribute' do
          backend_class1 = Class.new
          backend_class1.include(Translations::Backend)
          backend_class2 = Class.new
          backend_class2.include(Translations::Backend)
          ...
          expect(model_class.translation_backend_class('title')).to be < backend_class1
          expect(model_class.translation_backend_class('content')).to be < backend_class1
          expect(model_class.translation_backend_class('subtitle')).to be < backend_class2
        end
        ...
        it 'handles new backends added after first called' do
          ...
          expect(model_class.translation_backend_class('title')).to eq(mod.backend_class)

          other_mod = described_class.new('content', backend: :null)
          model_class.include other_mod

          expect(model_class.translation_backend_class('content')).to eq(other_mod.backend_class)
        end
      end
    end

    describe 'model instance methods' do
      describe '.translation_backends' do
        it 'returns instance of backend for attribute' do
          ...
          aggregate_failures do
            expect(article1.translation_backends).to eq({})
            expect(article2.translation_backends).to eq({})

            article1.title

            expect(article1.translation_backends.keys).to eq([:title])
            expect(article1.translation_backends[:title].class).to eq(mod1.backend_class)
            expect(article2.translation_backends.size).to eq(0)

            article2.content

            expect(article1.translation_backends.keys).to eq([:title])
            expect(article1.translation_backends[:title].class).to eq(mod1.backend_class)
            expect(article2.translation_backends.keys).to eq([:content])
            expect(article2.translation_backends[:content].class).to eq(mod2.backend_class)
          end
        end
        ...
        it 'maps string keys to symbol key values' do
          ...
          aggregate_failures do
            expect(article.translation_backends[:title]).to be_a(Translations::Backends::Null)
            expect(article.translation_backends['title']).to be_a(Translations::Backends::Null)
            expect(article.translation_backends.size).to eq(1)
            expect(article.translation_backends[:title]).to eq(article.translation_backends['title'])
          end
        end
    ...
    describe 'defining attribute backend on model' do
      ...
      it 'defines <attribute_name>_backend method which returns backend instance' do
        ...
        expect(article.translation_backends[:title]).to be_a(backend_class)
      end

      it 'memoizes backend instance' do
        expect(backend_class).to receive(:new).once.with(article, 'title').and_call_original
        2.times { article.translation_backends[:title] }
      end
    end
    ...
    describe 'defining getters and setters' do
      ...
      shared_examples_for 'reader' do
        it 'correctly maps getter method for translated attribute to backend' do
          ...
          I18n.with_locale(:de) { expect(article.title).to eq('foo') }
        end

        it 'correctly maps presence method for translated attribute to backend' do
          expect(listener).to receive(:read).with(:de, {}).and_return('foo')
          I18n.with_locale(:de) { expect(article.title?).to eq(true) }
        end
        ...
        it 'raises InvalidLocale exception if locale is not in I18n.available_locales' do
          expect { article.title(locale: :it) }.to raise_error(Translations::InvalidLocale)
        end

        it 'correctly maps other options to getter' do
          expect(listener).to receive(:read).with(:de, someopt: 'someval').and_return('foo')
          I18n.with_locale(:de) { expect(article.title(someopt: 'someval')).to eq('foo') }
        end
