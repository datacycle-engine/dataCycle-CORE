...
CREATE FUNCTION public.array_reverse(anyarray) RETURNS anyarray LANGUAGE SQL IMMUTABLE STRICT AS $_$
SELECT ARRAY(
    SELECT $1 [i]
...
CREATE FUNCTION public.compute_thing_schema_types(
  schema_types jsonb,
  template_name character varying DEFAULT NULL::character varying
) RETURNS character varying [] LANGUAGE plpgsql IMMUTABLE PARALLEL SAFE AS $$
...
CREATE FUNCTION public.concept_links_create_paths_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM upsert_ca_paths (ARRAY_AGG(new_concept_links.child_id))
...
CREATE FUNCTION public.concept_links_create_transitive_paths_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM upsert_ca_paths_transitive (ARRAY_AGG(new_concept_links.child_id))
FROM new_concept_links;
...
CREATE FUNCTION public.concept_links_update_paths_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM upsert_ca_paths (ARRAY_AGG(updated_concept_links.child_id))
FROM (
...
CREATE FUNCTION public.concept_links_update_transitive_paths_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM upsert_ca_paths_transitive (ARRAY_AGG(updated_concept_links.child_id))
FROM (
    SELECT DISTINCT new_concept_links.child_id
    FROM old_concept_links
      JOIN new_concept_links ON old_concept_links.id = new_concept_links.id
...
CREATE FUNCTION public.concept_schemes_update_paths_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM upsert_ca_paths (ARRAY_AGG(updated_concepts.id))
FROM (
...
CREATE FUNCTION public.concept_schemes_update_transitive_paths_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM upsert_ca_paths_transitive (ARRAY_AGG(updated_concepts.id))
FROM (
    SELECT DISTINCT concepts.id
    FROM old_concept_schemes
      JOIN new_concept_schemes ON old_concept_schemes.id = new_concept_schemes.id
...
CREATE FUNCTION public.concepts_create_paths_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM upsert_ca_paths (ARRAY_AGG(new_concepts.id))
FROM new_concepts;
...
CREATE FUNCTION public.concepts_create_transitive_paths_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM upsert_ca_paths_transitive (ARRAY_AGG(new_concepts.id))
FROM new_concepts;
...
CREATE FUNCTION public.concepts_update_paths_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM upsert_ca_paths (ARRAY_AGG(updated_concepts.id))
FROM (
...
CREATE FUNCTION public.concepts_update_transitive_paths_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM upsert_ca_paths_transitive (ARRAY_AGG(updated_concepts.id))
FROM (
    SELECT DISTINCT new_concepts.id
    FROM old_concepts
      JOIN new_concepts ON old_concepts.id = new_concepts.id
...
CREATE FUNCTION public.delete_ccc_relations_transitive_trigger_2() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM public.generate_ccc_from_ca_ids_transitive (array_agg(cccra.id))
FROM (
    SELECT DISTINCT ca.id
    FROM old_classification_alias_paths_transitive ocapt
      INNER JOIN classification_aliases ca ON ca.id = ANY (ocapt.full_path_ids)
...
CREATE FUNCTION public.delete_collected_classification_content_relations_trigger_1() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_collected_classification_content_relations (
  (
    ...
    WHERE classification_groups.classification_id = OLD.classification_id
  ),
  ARRAY []::uuid []
);
...
CREATE FUNCTION public.delete_concept_links_groups_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
...
CREATE FUNCTION public.delete_concept_links_groups_trigger_function2() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
DELETE FROM concept_links
WHERE concept_links.id IN (
    SELECT ocg.id
    FROM old_classification_groups ocg
...
CREATE FUNCTION public.delete_concept_links_to_histories_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$
...
CREATE FUNCTION public.delete_concept_links_trees_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
DELETE FROM concept_links
WHERE concept_links.id IN (
    SELECT nct.id
...
CREATE FUNCTION public.delete_concept_links_trees_trigger_function2() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
DELETE FROM concept_links
WHERE concept_links.id IN (
    SELECT oct.id
    FROM old_classification_trees oct
...
CREATE FUNCTION public.delete_concept_schemes_to_histories_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE insert_query TEXT;
...
CREATE FUNCTION public.delete_concept_schemes_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
...
CREATE FUNCTION public.delete_concepts_to_histories_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$
DECLARE insert_query TEXT;
...
CREATE FUNCTION public.delete_concepts_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
...
CREATE FUNCTION public.delete_external_hashes_trigger_1() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
...
CREATE FUNCTION public.delete_things_external_source_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
...
CREATE FUNCTION public.generate_ccc_from_ca_ids_transitive(ca_ids uuid []) RETURNS void LANGUAGE plpgsql AS $$ BEGIN IF array_length(ca_ids, 1) > 0 THEN WITH direct_classification_content_relations AS (
  SELECT DISTINCT ON (
...
CREATE FUNCTION public.generate_ccc_relations_transitive_trigger_1() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM public.generate_ccc_from_ca_ids_transitive (array_agg(cccra.id))
FROM (
    SELECT DISTINCT ca.id
    FROM new_classification_alias_paths_transitive ncapt
      INNER JOIN classification_aliases ca ON ca.id = ANY (ncapt.full_path_ids)
      AND ca.deleted_at IS NULL
  ) "cccra";
...
CREATE FUNCTION public.generate_collected_cl_content_relations_transitive(thing_ids uuid []) RETURNS void LANGUAGE plpgsql AS $$ BEGIN IF array_length(thing_ids, 1) > 0 THEN WITH direct_classification_content_relations AS (
  SELECT DISTINCT ON (
      classification_contents.content_data_id,
...
CREATE FUNCTION public.generate_collected_classification_content_relations(
  content_ids uuid [],
...
CREATE FUNCTION public.generate_collected_classification_content_relations_trigger_1() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_collected_classification_content_relations(
  ARRAY [NEW.content_data_id]::UUID [],
  ARRAY []::UUID []
);
...
CREATE FUNCTION public.generate_collected_classification_content_relations_trigger_2() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_collected_classification_content_relations(
...
CREATE FUNCTION public.generate_collected_classification_content_relations_trigger_3() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_collected_classification_content_relations (ARRAY_AGG(content_data_id), ARRAY []::uuid [])
FROM (
    ...
    WHERE classification_alias_paths.full_path_ids && ARRAY [NEW.id]::uuid []
...
CREATE FUNCTION public.generate_collected_classification_content_relations_trigger_4() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_collected_classification_content_relations (
  (
    SELECT ARRAY_AGG(DISTINCT things.id)
    FROM things
      JOIN classification_contents ON things.id = classification_contents.content_data_id
      JOIN classification_groups ON classification_contents.classification_id = classification_groups.classification_id
      AND classification_groups.deleted_at IS NULL
    WHERE classification_groups.classification_id = NEW.classification_id
  ),
  ARRAY []::uuid []
);
...
CREATE FUNCTION public.generate_collected_classification_content_relations_trigger_5() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_collected_classification_content_relations (ARRAY_AGG(content_data_id), ARRAY []::uuid [])
FROM (
    SELECT DISTINCT classification_contents.content_data_id
    FROM new_classification_alias_paths
      INNER JOIN classification_groups ON classification_groups.classification_alias_id = ANY (new_classification_alias_paths.full_path_ids)
      AND classification_groups.deleted_at IS NULL
      INNER JOIN classification_contents ON classification_contents.classification_id = classification_groups.classification_id
...
CREATE FUNCTION public.generate_collection_id_trigger() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN NEW.id := COALESCE(NEW.watch_list_id, NEW.stored_filter_id);
...
CREATE FUNCTION public.generate_content_content_links(content_content_ids uuid []) RETURNS void LANGUAGE plpgsql AS $$ BEGIN
...
FROM content_contents
WHERE content_contents.id = ANY(content_content_ids) ON CONFLICT (
    content_content_id,
    content_a_id,
    content_b_id,
    relation
...
CREATE FUNCTION public.generate_content_content_links_trigger() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_content_content_links (
...
CREATE FUNCTION public.generate_content_content_links_trigger2() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_content_content_links (
...
CREATE FUNCTION public.generate_my_selection_watch_list() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN IF EXISTS (
  SELECT
  FROM roles
...
CREATE FUNCTION public.generate_schedule_occurences_array(
...
WHEN s_rrule IS NOT NULL THEN all_occurrences := get_occurrences (
  (
    CASE
      ...
      ELSE (s_rrule || ';UNTIL=2029-08-01')
    END
  )::rrule,
  s_dtstart AT TIME ZONE 'Europe/Vienna',
  '2029-08-01' AT TIME ZONE 'Europe/Vienna'
);
...
WHERE occurences.occurence IS NOT NULL
  AND occurences.occurence + schedule_duration > '2023-08-01'
  AND NOT EXISTS (
    SELECT 1
    FROM exdates
...
CREATE FUNCTION public.generate_unique_collection_slug(
  old_slug character varying,
  OUT new_slug character varying
) RETURNS character varying LANGUAGE plpgsql AS $_$ BEGIN WITH input AS (
...
CREATE FUNCTION public.geom_simple_update() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN NEW.geom_simple := (
  st_simplify(
...
CREATE FUNCTION public.get_dict(lang character varying) RETURNS regconfig LANGUAGE plpgsql IMMUTABLE STRICT PARALLEL SAFE AS $$
...
CREATE FUNCTION public.insert_classification_trees_order_a_trigger() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM reset_classification_aliases_order_a(ARRAY_AGG(classification_alias_id))
FROM (
...
CREATE FUNCTION public.insert_concept_links_trees_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN WITH updated_concepts AS (
  UPDATE concepts
...
CREATE FUNCTION public.insert_concept_schemes_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
...
CREATE FUNCTION public.insert_concepts_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
...
CREATE FUNCTION public.reset_classification_aliases_order_a(classification_alias_ids uuid []) RETURNS void LANGUAGE plpgsql AS $$ BEGIN IF array_length(classification_alias_ids, 1) > 0 THEN
...
CREATE FUNCTION public.to_classification_content_history(content_id uuid, new_history_id uuid) RETURNS void LANGUAGE plpgsql AS $$
DECLARE insert_query TEXT;
...
CREATE FUNCTION public.to_content_collection_link_history(content_id uuid, new_history_id uuid) RETURNS void LANGUAGE plpgsql AS $$
DECLARE insert_query TEXT;
...
CREATE FUNCTION public.to_content_content_history(
  content_id uuid,
  new_history_id uuid,
...
WHERE table_name = 'content_content_histories'
  AND column_name NOT IN (
    'id',
    'content_a_history_id',
    'content_b_history_id',
    'content_b_history_type'
  );
...
CREATE FUNCTION public.to_schedule_history(content_id uuid, new_history_id uuid) RETURNS void LANGUAGE plpgsql AS $$
DECLARE insert_query TEXT;
...
CREATE FUNCTION public.to_thing_history(
  content_id uuid,
  current_locale character varying,
  all_translations boolean DEFAULT false,
  deleted boolean DEFAULT false
...
CREATE FUNCTION public.to_thing_history_translation(
  content_id uuid,
  new_history_id uuid,
  current_locale character varying,
  all_translations boolean DEFAULT false
...
CREATE FUNCTION public.tsvectorsearchupdate() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN NEW.words := to_tsvector(NEW.dict, NEW.full_text::text);
...
CREATE FUNCTION public.update_classification_aliases_order_a(tree_label_ids uuid []) RETURNS void LANGUAGE plpgsql AS $$ BEGIN IF array_length(tree_label_ids, 1) > 0 THEN
UPDATE classification_aliases
...
FROM (
    WITH RECURSIVE paths (id, updated_at, full_order_a, tree_label_id) AS (
      SELECT classification_aliases.id,
        ...
        ARRAY [ ( ROW_NUMBER() OVER ( PARTITION BY classification_trees.classification_tree_label_id ORDER BY classification_aliases.order_a ASC, classification_aliases.updated_at ASC ) ) ],
        classification_trees.classification_tree_label_id
...
WHERE w.id = classification_aliases.id
  AND classification_aliases.order_a IS DISTINCT
FROM w.order_a;
...
CREATE FUNCTION public.update_classification_aliases_order_a_trigger() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM update_classification_aliases_order_a (ARRAY_AGG(classification_tree_label_id))
FROM (
...
CREATE FUNCTION public.update_classification_tree_tree_label_id(classification_tree_ids uuid []) RETURNS void LANGUAGE plpgsql AS $$ BEGIN IF array_length(classification_tree_ids, 1) > 0 THEN
...
CREATE FUNCTION public.update_classification_tree_tree_label_id_concept_trigger() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
UPDATE concepts
...
CREATE FUNCTION public.update_classification_tree_tree_label_id_trigger() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM update_classification_tree_tree_label_id (ARRAY_AGG(id))
FROM (
    SELECT new_classification_trees.id
    FROM new_classification_trees
...
CREATE FUNCTION public.update_classification_trees_order_a_trigger() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM reset_classification_aliases_order_a(ARRAY_AGG(classification_alias_id))
FROM (
    SELECT new_classification_trees.classification_alias_id
    FROM new_classification_trees
      INNER JOIN old_classification_trees ON old_classification_trees.id = new_classification_trees.id
...
CREATE FUNCTION public.update_collected_classification_content_relations_trigger_4() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_collected_classification_content_relations (
  (
    SELECT ARRAY_AGG(DISTINCT things.id)
    FROM things
      JOIN classification_contents ON things.id = classification_contents.content_data_id
...
CREATE FUNCTION public.update_concept_links_groups_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
...
CREATE FUNCTION public.update_concept_links_trees_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
UPDATE concept_links
...
CREATE FUNCTION public.update_concept_schemes_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
...
CREATE FUNCTION public.update_concepts_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
UPDATE concepts
...
CREATE FUNCTION public.update_dict_in_searches() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN NEW.dict = get_dict(NEW.locale);
...
CREATE FUNCTION public.update_template_definitions_trigger() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN
UPDATE things
...
CREATE FUNCTION public.update_template_name_dependent_in_things() RETURNS TRIGGER LANGUAGE plpgsql AS $$
...
CREATE FUNCTION public.upsert_ca_paths(concept_ids uuid []) RETURNS void LANGUAGE plpgsql AS $$ BEGIN IF array_length(concept_ids, 1) > 0 THEN WITH RECURSIVE paths(
  id,
  parent_id,
  ancestor_ids,
  full_path_ids,
...
CREATE FUNCTION public.upsert_ca_paths_transitive(concept_ids uuid []) RETURNS void LANGUAGE plpgsql AS $$ BEGIN IF array_length(concept_ids, 1) > 0 THEN WITH RECURSIVE paths(
  id,
  parent_id,
  ancestor_ids,
  full_path_ids,
...
CREATE FUNCTION public.upsert_concept_tables_trigger_function() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN WITH groups AS (
  SELECT cg.*,
    (
      (
...
CREATE FUNCTION public.websearch_to_prefix_tsquery(regconfig, text) RETURNS tsquery LANGUAGE plpgsql IMMUTABLE STRICT COST 101 PARALLEL SAFE AS $_$
...
CREATE VIEW public.classification_alias_links AS WITH primary_classification_groups AS (
  SELECT DISTINCT classification_groups.classification_id,
    first_value(classification_groups.classification_alias_id) OVER (
      ...
      ORDER BY classification_groups.created_at
...
FROM (
    ...
    JOIN public.classification_groups additional_classification_groups ON (
      (
        (
          primary_classification_groups.classification_id = additional_classification_groups.classification_id
        )
        AND (
          additional_classification_groups.classification_alias_id <> primary_classification_groups.classification_alias_id
        )
        AND (
          additional_classification_groups.deleted_at IS NULL
        )
      )
    )
  )
...
CREATE TABLE public.collection_shares (
  ...
  user_id uuid GENERATED ALWAYS AS (
    CASE
      WHEN (
        (shareable_type)::text = 'DataCycleCore::User'::text
      ) THEN shareable_id
      ELSE NULL::uuid
    END
  ) STORED,
  user_group_id uuid GENERATED ALWAYS AS (
    CASE
      WHEN (
        (shareable_type)::text = 'DataCycleCore::UserGroup'::text
      ) THEN shareable_id
      ELSE NULL::uuid
    END
  ) STORED,
  role_id uuid GENERATED ALWAYS AS (
    CASE
      WHEN (
        (shareable_type)::text = 'DataCycleCore::Role'::text
      ) THEN shareable_id
      ELSE NULL::uuid
    END
  ) STORED
    ...
    (
      (
        setweight(
          ...
          to_tsvector(
            'simple'::regconfig,
            (COALESCE(slug, ''::character varying))::text
          ),
          'B'::"char"
        )
      ) || setweight(
        to_tsvector(
          'simple'::regconfig,
          COALESCE(description_stripped, ''::text)
        ),
        'C'::"char"
      )
    )
...
CREATE TABLE public.thing_templates (
  ...
  created_at timestamp without time zone DEFAULT transaction_timestamp() NOT NULL,
  updated_at timestamp without time zone DEFAULT transaction_timestamp() NOT NULL,
  computed_schema_types character varying [] GENERATED ALWAYS AS (
    public.compute_thing_schema_types(
      (schema->'schema_ancestors'::text),
      template_name
    )
  ) STORED
);
...
FROM (
    public.data_links
    JOIN public.watch_list_data_hashes ON (
      (
        watch_list_data_hashes.watch_list_id = data_links.item_id
      )
    )
  )
WHERE (
    (data_links.item_type)::text = 'DataCycleCore::WatchList'::text
  )
UNION
SELECT data_links.id AS data_link_id,
...
FROM (
    public.things
    JOIN public.thing_templates ON (
      (
        (thing_templates.template_name)::text = (things.template_name)::text
      )
    )
  );
...
FROM (
    ...
    JOIN public.content_computed_properties ON (
      (
        (
          (content_computed_properties.template_name)::text = (t2.template_name)::text
        )
        AND (
          content_computed_properties.compute_parameter_property_name = (content_contents.relation_a)::text
        )
      )
    )
  )
...
FROM (
    (
      (
        public.things
        JOIN public.content_contents ON (
          (
            (content_contents.content_a_id = things.id)
    ...
    JOIN public.content_computed_properties ON (
      (
        ...
        AND (
          content_computed_properties.compute_parameter_property_name = (content_contents.relation_b)::text
        )
      )
    )
  );
...
SELECT queue AS queue_name,
  ...
  array_agg(DISTINCT delayed_reference_type) FILTER (
    WHERE (failed_at IS NOT NULL)
  ) AS failed_types,
  array_agg(DISTINCT delayed_reference_type) FILTER (
    WHERE (
...
CREATE TABLE public.schedules (
  ...
  updated_at timestamp without time zone NOT NULL,
  holidays boolean,
  occurrences tstzmultirange GENERATED ALWAYS AS (
    public.generate_schedule_occurences_array(dtstart, rrule, rdate, exdate, duration)
  ) STORED
);
      ...
      (
        (
          setweight(
            to_tsvector(
              dict,
              (COALESCE(headline, ''::character varying))::text
            ),
            'A'::"char"
          ) || setweight(
            to_tsvector(
          ...
          to_tsvector(
            dict,
            (
              COALESCE(classification_string, ''::character varying)
            )::text
          ),
          'C'::"char"
        )
      ) || setweight(
        to_tsvector(dict, COALESCE(full_text, ''::text)),
        'D'::"char"
      )
    )
  ) STORED
...
ALTER COLUMN id
SET DEFAULT nextval('public.delayed_jobs_id_seq'::regclass);
...
ADD CONSTRAINT content_content_links_uq_constraint UNIQUE (
    content_content_id,
    content_a_id,
    content_b_id,
    relation
...
CREATE UNIQUE INDEX child_parent_index ON public.classification_trees USING btree (
  classification_alias_id,
...
CREATE UNIQUE INDEX classification_groups_ca_id_c_id_uq_idx ON public.classification_groups USING btree (classification_alias_id, classification_id)
...
CREATE UNIQUE INDEX index_classification_aliases_unique_external_source_id_and_key ON public.classification_aliases USING btree (external_source_id, external_key)
WHERE (deleted_at IS NULL);
...
CREATE UNIQUE INDEX index_classification_trees_unique_classification_alias ON public.classification_trees USING btree (classification_alias_id)
WHERE (deleted_at IS NULL);
...
CREATE UNIQUE INDEX index_classifications_unique_external_source_id_and_key ON public.classifications USING btree (external_source_id, external_key)
WHERE (deleted_at IS NULL);
...
CREATE UNIQUE INDEX index_concept_links_on_child_id ON public.concept_links USING btree (child_id)
WHERE ((link_type)::text = 'broader'::text);
...
CREATE UNIQUE INDEX index_concepts_on_external_system_id_and_external_key ON public.concepts USING btree (external_system_id, external_key)
WHERE (
    (external_system_id IS NOT NULL)
...
CREATE UNIQUE INDEX index_ctl_on_external_source_id_and_external_key ON public.classification_tree_labels USING btree (external_source_id, external_key)
WHERE (deleted_at IS NULL);
...
CREATE UNIQUE INDEX index_external_system_syncs_on_unique_attributes ON public.external_system_syncs USING btree (
  syncable_type,
  syncable_id,
  external_system_id,
  sync_type,
...
CREATE UNIQUE INDEX parent_child_index ON public.classification_trees USING btree (
  parent_classification_alias_id,
  classification_alias_id
);
...
CREATE UNIQUE INDEX unique_thing_duplicate_idx ON public.thing_duplicates USING btree (
...
CREATE TRIGGER concept_links_create_paths_trigger
AFTER
...
AFTER
INSERT ON public.concept_links REFERENCING NEW TABLE AS new_concept_links FOR EACH STATEMENT EXECUTE FUNCTION public.concept_links_create_transitive_paths_trigger_function();
...
AFTER DELETE ON public.concept_links REFERENCING OLD TABLE AS old_concept_links FOR EACH STATEMENT EXECUTE FUNCTION public.concept_links_delete_transitive_paths_trigger_function();
...
CREATE TRIGGER concept_links_update_paths_trigger
AFTER
UPDATE ON public.concept_links REFERENCING OLD TABLE AS old_concept_links NEW TABLE AS new_concept_links FOR EACH STATEMENT EXECUTE FUNCTION public.concept_links_update_paths_trigger_function();
...
AFTER
UPDATE ON public.concept_links REFERENCING OLD TABLE AS old_concept_links NEW TABLE AS new_concept_links FOR EACH STATEMENT EXECUTE FUNCTION public.concept_links_update_transitive_paths_trigger_function();
...
CREATE TRIGGER concept_schemes_update_paths_trigger
AFTER
UPDATE ON public.concept_schemes REFERENCING OLD TABLE AS old_concept_schemes NEW TABLE AS new_concept_schemes FOR EACH STATEMENT EXECUTE FUNCTION public.concept_schemes_update_paths_trigger_function();
...
AFTER
UPDATE ON public.concept_schemes REFERENCING OLD TABLE AS old_concept_schemes NEW TABLE AS new_concept_schemes FOR EACH STATEMENT EXECUTE FUNCTION public.concept_schemes_update_transitive_paths_trigger_function();
...
CREATE TRIGGER concepts_create_paths_trigger
AFTER
...
AFTER
INSERT ON public.concepts REFERENCING NEW TABLE AS new_concepts FOR EACH STATEMENT EXECUTE FUNCTION public.concepts_create_transitive_paths_trigger_function();
...
AFTER DELETE ON public.concepts REFERENCING OLD TABLE AS old_concepts FOR EACH STATEMENT EXECUTE FUNCTION public.concepts_delete_transitive_paths_trigger_function();
...
CREATE TRIGGER concepts_update_paths_trigger
AFTER
UPDATE ON public.concepts REFERENCING OLD TABLE AS old_concepts NEW TABLE AS new_concepts FOR EACH STATEMENT EXECUTE FUNCTION public.concepts_update_paths_trigger_function();
...
AFTER
UPDATE ON public.concepts REFERENCING OLD TABLE AS old_concepts NEW TABLE AS new_concepts FOR EACH STATEMENT EXECUTE FUNCTION public.concepts_update_transitive_paths_trigger_function();
...
AFTER DELETE ON public.classification_alias_paths_transitive REFERENCING OLD TABLE AS old_classification_alias_paths_transitive FOR EACH STATEMENT EXECUTE FUNCTION public.delete_ccc_relations_transitive_trigger_2();
...
CREATE TRIGGER delete_ccc_relations_transitive_trigger
AFTER DELETE ON public.classification_contents FOR EACH ROW EXECUTE FUNCTION public.delete_ccc_relations_transitive_trigger_1();
...
AFTER DELETE ON public.classification_groups FOR EACH ROW EXECUTE FUNCTION public.delete_collected_classification_content_relations_trigger_1();
...
AFTER
UPDATE ON public.classification_groups REFERENCING OLD TABLE AS old_classification_groups NEW TABLE AS new_classification_groups FOR EACH STATEMENT EXECUTE FUNCTION public.delete_concept_links_groups_trigger_function();
...
AFTER DELETE ON public.classification_groups REFERENCING OLD TABLE AS old_classification_groups FOR EACH STATEMENT EXECUTE FUNCTION public.delete_concept_links_groups_trigger_function2();
...
AFTER DELETE ON public.concept_links REFERENCING OLD TABLE AS old_concept_links FOR EACH STATEMENT EXECUTE FUNCTION public.delete_concept_links_to_histories_trigger_function();
...
AFTER
UPDATE ON public.classification_trees REFERENCING OLD TABLE AS old_classification_trees NEW TABLE AS new_classification_trees FOR EACH STATEMENT EXECUTE FUNCTION public.delete_concept_links_trees_trigger_function();
...
AFTER DELETE ON public.classification_trees REFERENCING OLD TABLE AS old_classification_trees FOR EACH STATEMENT EXECUTE FUNCTION public.delete_concept_links_trees_trigger_function2();
...
AFTER DELETE ON public.concept_schemes REFERENCING OLD TABLE AS old_concept_schemes FOR EACH STATEMENT EXECUTE FUNCTION public.delete_concept_schemes_to_histories_trigger_function();
...
AFTER
UPDATE ON public.classification_tree_labels REFERENCING OLD TABLE AS old_classification_tree_labels NEW TABLE AS new_classification_tree_labels FOR EACH STATEMENT EXECUTE FUNCTION public.delete_concept_schemes_trigger_function();
...
AFTER DELETE ON public.concepts REFERENCING OLD TABLE AS old_concepts FOR EACH STATEMENT EXECUTE FUNCTION public.delete_concepts_to_histories_trigger_function();
...
AFTER
UPDATE ON public.classification_aliases REFERENCING OLD TABLE AS old_classification_aliases NEW TABLE AS new_classification_aliases FOR EACH STATEMENT EXECUTE FUNCTION public.delete_concepts_trigger_function();
...
AFTER DELETE ON public.thing_translations REFERENCING OLD TABLE AS old_thing_translations FOR EACH STATEMENT EXECUTE FUNCTION public.delete_external_hashes_trigger_1();
...
UPDATE OF external_key,
  external_source_id ON public.things FOR EACH ROW
  WHEN (
    (
      (
      ...
      OR (
        (
          old.external_source_id IS DISTINCT
...
INSERT ON public.searches FOR EACH ROW EXECUTE FUNCTION public.update_dict_in_searches();
...
UPDATE OF locale ON public.searches FOR EACH ROW
  WHEN (
    (
      (old.locale)::text IS DISTINCT
      FROM (new.locale)::text
    )
  ) EXECUTE FUNCTION public.update_dict_in_searches();
...
AFTER
INSERT ON public.classification_alias_paths_transitive REFERENCING NEW TABLE AS new_classification_alias_paths_transitive FOR EACH STATEMENT EXECUTE FUNCTION public.generate_ccc_relations_transitive_trigger_1();
...
CREATE TRIGGER generate_ccc_relations_transitive_trigger
AFTER
INSERT ON public.classification_contents FOR EACH ROW EXECUTE FUNCTION public.generate_ccc_relations_transitive_trigger_2();
...
AFTER
INSERT ON public.classification_alias_paths REFERENCING NEW TABLE AS new_classification_alias_paths FOR EACH STATEMENT EXECUTE FUNCTION public.generate_collected_classification_content_relations_trigger_5();
...
AFTER
INSERT ON public.classification_contents FOR EACH ROW EXECUTE FUNCTION public.generate_collected_classification_content_relations_trigger_1();
...
AFTER DELETE ON public.classification_contents FOR EACH ROW EXECUTE FUNCTION public.generate_collected_classification_content_relations_trigger_2();
...
AFTER
INSERT ON public.classification_groups FOR EACH ROW EXECUTE FUNCTION public.generate_collected_classification_content_relations_trigger_4();
...
INSERT ON public.collections FOR EACH ROW
  WHEN ((new.slug IS NOT NULL)) EXECUTE FUNCTION public.generate_collection_slug_trigger();
...
AFTER
INSERT ON public.content_contents REFERENCING NEW TABLE AS new_content_contents FOR EACH STATEMENT EXECUTE FUNCTION public.generate_content_content_links_trigger();
...
AFTER
INSERT ON public.users FOR EACH ROW EXECUTE FUNCTION public.generate_my_selection_watch_list();
...
INSERT ON public.things FOR EACH ROW EXECUTE FUNCTION public.geom_simple_update();
...
UPDATE OF location,
  line,
  geom ON public.things FOR EACH ROW
  WHEN (
    (
      (
      ...
      OR (
        (old.line)::text IS DISTINCT
        FROM (new.line)::text
      )
      OR (
        (old.geom)::text IS DISTINCT
        FROM (new.geom)::text
      )
    )
  ) EXECUTE FUNCTION public.geom_simple_update();
...
AFTER
INSERT ON public.classification_trees REFERENCING NEW TABLE AS new_classification_trees FOR EACH STATEMENT EXECUTE FUNCTION public.insert_classification_trees_order_a_trigger();
...
AFTER
INSERT ON public.classification_trees REFERENCING NEW TABLE AS new_classification_trees FOR EACH STATEMENT EXECUTE FUNCTION public.insert_concept_links_trees_trigger_function();
...
AFTER
INSERT ON public.classification_tree_labels REFERENCING NEW TABLE AS new_classification_tree_labels FOR EACH STATEMENT EXECUTE FUNCTION public.insert_concept_schemes_trigger_function();
...
AFTER
INSERT ON public.classification_aliases REFERENCING NEW TABLE AS new_classification_aliases FOR EACH STATEMENT EXECUTE FUNCTION public.insert_concepts_trigger_function();

--
-- Name: searches tsvectorsearchinsert; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER tsvectorsearchinsert BEFORE
INSERT ON public.searches FOR EACH ROW EXECUTE FUNCTION public.tsvectorsearchupdate();

--
-- Name: searches tsvectorsearchupdate; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER tsvectorsearchupdate BEFORE
UPDATE OF words,
  locale,
  full_text ON public.searches FOR EACH ROW
  WHEN (
    (
      (
        old.words IS DISTINCT
        FROM new.words
      )
      OR (
        (old.locale)::text IS DISTINCT
      ...
      OR (
        old.full_text IS DISTINCT
        FROM new.full_text
      )
    )
  ) EXECUTE FUNCTION public.tsvectorsearchupdate();

--
-- Name: searches tsvectortypeaheadsearchinsert; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER tsvectortypeaheadsearchinsert BEFORE
INSERT ON public.searches FOR EACH ROW EXECUTE FUNCTION tsvector_update_trigger(
    'words_typeahead',
    'pg_catalog.simple',
    'full_text'
  );
...
CREATE TRIGGER tsvectortypeaheadsearchupdate BEFORE
UPDATE OF full_text ON public.searches FOR EACH ROW
  WHEN (
    (
      old.full_text IS DISTINCT
      FROM new.full_text
...
AFTER
UPDATE OF content_data_id,
  classification_id,
  relation ON public.classification_contents FOR EACH ROW
  WHEN (
    (
      (
        old.content_data_id IS DISTINCT
        FROM new.content_data_id
      )
      OR (
      ...
      OR (
        (old.relation)::text IS DISTINCT
        FROM (new.relation)::text
      )
    )
  ) EXECUTE FUNCTION public.generate_ccc_relations_transitive_trigger_2();
...
UPDATE OF classification_id,
  classification_alias_id ON public.classification_groups FOR EACH ROW
  WHEN (
    (
      (
        old.classification_id IS DISTINCT
  ...
  ) EXECUTE FUNCTION public.update_collected_classification_content_relations_trigger_4();
...
AFTER
UPDATE ON public.classification_aliases REFERENCING OLD TABLE AS old_classification_aliases NEW TABLE AS updated_classification_aliases FOR EACH STATEMENT EXECUTE FUNCTION public.update_classification_aliases_order_a_trigger();
...
AFTER
UPDATE ON public.classification_trees REFERENCING OLD TABLE AS old_classification_trees NEW TABLE AS new_classification_trees FOR EACH STATEMENT EXECUTE FUNCTION public.update_classification_trees_order_a_trigger();

--
-- Name: classification_trees update_classification_tree_tree_label_id_concept; Type: TRIGGER; Schema: public; Owner: -
--

CREATE TRIGGER update_classification_tree_tree_label_id_concept
AFTER
UPDATE ON public.classification_trees REFERENCING OLD TABLE AS old_classification_trees NEW TABLE AS new_classification_trees FOR EACH STATEMENT EXECUTE FUNCTION public.update_classification_tree_tree_label_id_concept_trigger();
...
AFTER
UPDATE ON public.classification_trees REFERENCING OLD TABLE AS old_classification_trees NEW TABLE AS new_classification_trees FOR EACH STATEMENT EXECUTE FUNCTION public.update_classification_tree_tree_label_id_trigger();
...
AFTER
UPDATE ON public.classification_alias_paths FOR EACH ROW EXECUTE FUNCTION public.generate_collected_classification_content_relations_trigger_3();
...
UPDATE OF content_data_id,
  ...
  ) EXECUTE FUNCTION public.generate_collected_classification_content_relations_trigger_1();
...
UPDATE OF slug ON public.collections FOR EACH ROW
  WHEN (
    (
      (new.slug IS NOT NULL)
      AND (
        (old.slug)::text IS DISTINCT
        FROM (new.slug)::text
      )
    )
  ) EXECUTE FUNCTION public.generate_collection_slug_trigger();
...
AFTER
UPDATE ON public.classification_groups REFERENCING OLD TABLE AS old_classification_groups NEW TABLE AS new_classification_groups FOR EACH STATEMENT EXECUTE FUNCTION public.update_concept_links_groups_trigger_function();
...
AFTER
UPDATE ON public.classification_trees REFERENCING OLD TABLE AS old_classification_trees NEW TABLE AS new_classification_trees FOR EACH STATEMENT EXECUTE FUNCTION public.update_concept_links_trees_trigger_function();
...
AFTER
UPDATE ON public.classification_tree_labels REFERENCING OLD TABLE AS old_classification_tree_labels NEW TABLE AS new_classification_tree_labels FOR EACH STATEMENT EXECUTE FUNCTION public.update_concept_schemes_trigger_function();
...
AFTER
UPDATE ON public.classification_aliases REFERENCING OLD TABLE AS old_classification_aliases NEW TABLE AS new_classification_aliases FOR EACH STATEMENT EXECUTE FUNCTION public.update_concepts_trigger_function();
...
AFTER
UPDATE ON public.content_contents REFERENCING OLD TABLE AS old_content_contents NEW TABLE AS new_content_contents FOR EACH STATEMENT EXECUTE FUNCTION public.generate_content_content_links_trigger2();
...
AFTER
UPDATE OF deleted_at ON public.classification_groups FOR EACH ROW
  WHEN (
    (
      (old.deleted_at IS NULL)
      AND (new.deleted_at IS NOT NULL)
    )
  ) EXECUTE FUNCTION public.delete_collected_classification_content_relations_trigger_1();
...
AFTER
UPDATE OF role_id ON public.users FOR EACH ROW
  WHEN (
    (
      old.role_id IS DISTINCT
      FROM new.role_id
    )
  ) EXECUTE FUNCTION public.generate_my_selection_watch_list();
...
AFTER
UPDATE ON public.thing_templates REFERENCING OLD TABLE AS old_thing_templates NEW TABLE AS new_thing_templates FOR EACH STATEMENT EXECUTE FUNCTION public.update_template_definitions_trigger();
...
UPDATE OF template_name ON public.things FOR EACH ROW
  ...
  ) EXECUTE FUNCTION public.update_template_name_dependent_in_things();
...
AFTER
INSERT ON public.classification_groups REFERENCING NEW TABLE AS new_classification_groups FOR EACH STATEMENT EXECUTE FUNCTION public.upsert_concept_tables_trigger_function();
...
ALTER TABLE ONLY public.classification_trees
ADD CONSTRAINT fk_rails_0aeb2f8fa2 FOREIGN KEY (external_source_id) REFERENCES public.external_systems(id) ON DELETE
SET NULL NOT VALID;
...
ALTER TABLE ONLY public.thing_histories
ADD CONSTRAINT fk_rails_71ac418654 FOREIGN KEY (external_source_id) REFERENCES public.external_systems(id) ON DELETE
SET NULL NOT VALID;
...
ALTER TABLE ONLY public.classifications
ADD CONSTRAINT fk_rails_72385dbd06 FOREIGN KEY (external_source_id) REFERENCES public.external_systems(id) ON DELETE
SET NULL NOT VALID;
...
ALTER TABLE ONLY public.things
ADD CONSTRAINT fk_rails_7b61990cb0 FOREIGN KEY (external_source_id) REFERENCES public.external_systems(id) ON DELETE
SET NULL NOT VALID;
...
ALTER TABLE ONLY public.collections
ADD CONSTRAINT fk_rails_9b33697360 FOREIGN KEY (user_id) REFERENCES public.users(id) ON UPDATE CASCADE ON DELETE
SET NULL;
...
ALTER TABLE ONLY public.classification_aliases
ADD CONSTRAINT fk_rails_a7798aa495 FOREIGN KEY (external_source_id) REFERENCES public.external_systems(id) ON DELETE
SET NULL NOT VALID;
...
ALTER TABLE ONLY public.collections
ADD CONSTRAINT fk_rails_f092282905 FOREIGN KEY (linked_stored_filter_id) REFERENCES public.collections(id) ON UPDATE CASCADE ON DELETE
SET NULL;
...
ALTER TABLE ONLY public.classification_groups
ADD CONSTRAINT fk_rails_f570600b17 FOREIGN KEY (external_source_id) REFERENCES public.external_systems(id) ON DELETE
SET NULL NOT VALID;
