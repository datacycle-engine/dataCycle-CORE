...
CREATE FUNCTION public.delete_ccc_relations_transitive_trigger_2() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_collected_cl_content_relations_transitive (
  array_agg(
...
CREATE FUNCTION public.generate_ccc_relations_transitive_trigger_1() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_collected_cl_content_relations_transitive (
  array_agg(
    collected_classification_content_relations_alias.content_data_id
  )
)
...
CREATE FUNCTION public.generate_classification_alias_paths(classification_alias_ids uuid []) RETURNS void LANGUAGE plpgsql AS $$ BEGIN
...
CREATE FUNCTION public.generate_classification_alias_paths_trigger_1() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_classification_alias_paths (array_agg(id) || ARRAY [NEW.id]::UUID [])
FROM (
    SELECT id
...
CREATE FUNCTION public.generate_classification_alias_paths_trigger_2() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_classification_alias_paths (
...
CREATE FUNCTION public.generate_classification_alias_paths_trigger_3() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM generate_classification_alias_paths (array_agg(classification_alias_id))
FROM (
...
WHEN s_rrule IS NOT NULL THEN all_occurrences := get_occurrences (
  (
    CASE
      ...
      ELSE (s_rrule || ';UNTIL=2029-06-20')
    END
  )::rrule,
  s_dtstart AT TIME ZONE 'Europe/Vienna',
  '2029-06-20' AT TIME ZONE 'Europe/Vienna'
);
...
WHERE occurences.occurence IS NOT NULL
  AND occurences.occurence + schedule_duration > '2023-06-20'
  AND NOT EXISTS (
    SELECT 1
    FROM exdates
...
CREATE UNIQUE INDEX index_concept_link_histories_on_child_id ON public.concept_link_histories USING btree (child_id)
