...
CREATE FUNCTION public.concept_links_create_paths_trigger_function() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM upsert_ca_paths (ARRAY_AGG(new_concept_links.child_id))
...
CREATE FUNCTION public.concept_links_create_transitive_paths_trigger_function() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM upsert_ca_paths_transitive (ARRAY_AGG(new_concept_links.child_id))
FROM new_concept_links;
...
CREATE FUNCTION public.concept_links_delete_transitive_paths_trigger_function() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM upsert_ca_paths_transitive (ARRAY_AGG(old_concept_links.child_id))
...
CREATE FUNCTION public.concept_links_update_paths_trigger_function() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM upsert_ca_paths (ARRAY_AGG(updated_concept_links.child_id))
FROM (
...
CREATE FUNCTION public.concept_links_update_transitive_paths_trigger_function() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM upsert_ca_paths_transitive (ARRAY_AGG(updated_concept_links.child_id))
FROM (
    SELECT DISTINCT new_concept_links.child_id
    FROM old_concept_links
      JOIN new_concept_links ON old_concept_links.id = new_concept_links.id
...
CREATE FUNCTION public.concept_schemes_update_paths_trigger_function() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM upsert_ca_paths (ARRAY_AGG(updated_concepts.id))
FROM (
...
CREATE FUNCTION public.concept_schemes_update_transitive_paths_trigger_function() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM upsert_ca_paths_transitive (ARRAY_AGG(updated_concepts.id))
FROM (
    SELECT DISTINCT concepts.id
    FROM old_concept_schemes
      JOIN new_concept_schemes ON old_concept_schemes.id = new_concept_schemes.id
...
CREATE FUNCTION public.concepts_create_paths_trigger_function() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM upsert_ca_paths (ARRAY_AGG(new_concepts.id))
FROM new_concepts;
...
CREATE FUNCTION public.concepts_create_transitive_paths_trigger_function() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM upsert_ca_paths_transitive (ARRAY_AGG(new_concepts.id))
FROM new_concepts;
...
CREATE FUNCTION public.concepts_delete_transitive_paths_trigger_function() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM upsert_ca_paths_transitive (ARRAY_AGG(old_concepts.id))
FROM old_concepts;
...
CREATE FUNCTION public.delete_ccc_relations_transitive_trigger_1() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM generate_collected_cl_content_relations_transitive (ARRAY [OLD.content_data_id]::UUID []);
...
CREATE FUNCTION public.delete_ccc_relations_transitive_trigger_2() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM public.generate_ccc_from_ca_ids_transitive (array_agg(cccra.id))
FROM (
...
CREATE FUNCTION public.generate_ccc_relations_transitive_trigger_2() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM generate_collected_cl_content_relations_transitive (ARRAY [NEW.content_data_id]::UUID []);
...
CREATE FUNCTION public.generate_collected_cl_content_relations_transitive(thing_ids uuid[]) RETURNS void
    LANGUAGE plpgsql
    AS $$ BEGIN IF array_length(thing_ids, 1) > 0 THEN WITH direct_classification_content_relations AS (
  SELECT DISTINCT ON (
...
CREATE FUNCTION public.generate_collection_id_trigger() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN NEW.id := COALESCE(NEW.watch_list_id, NEW.stored_filter_id);
...
CREATE FUNCTION public.generate_collection_slug_trigger() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN NEW.slug := generate_unique_collection_slug (NEW.slug);
...
CREATE FUNCTION public.insert_classification_trees_order_a_trigger() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM reset_classification_aliases_order_a(ARRAY_AGG(classification_alias_id))
FROM (
...
CREATE FUNCTION public.insert_concept_links_trees_trigger_function() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN WITH updated_concepts AS (
  UPDATE concepts
...
CREATE FUNCTION public.reset_classification_aliases_order_a(classification_alias_ids uuid[]) RETURNS void
    LANGUAGE plpgsql
    AS $$ BEGIN IF array_length(classification_alias_ids, 1) > 0 THEN
...
CREATE FUNCTION public.tsvectorsearchupdate() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN NEW.words := to_tsvector(NEW.dict, NEW.full_text::text);
...
CREATE FUNCTION public.update_classification_aliases_order_a(tree_label_ids uuid[]) RETURNS void
    LANGUAGE plpgsql
    AS $$ BEGIN IF array_length(tree_label_ids, 1) > 0 THEN
UPDATE classification_aliases
...
CREATE FUNCTION public.update_classification_tree_tree_label_id(classification_tree_ids uuid[]) RETURNS void
    LANGUAGE plpgsql
    AS $$ BEGIN IF array_length(classification_tree_ids, 1) > 0 THEN
...
CREATE FUNCTION public.update_classification_tree_tree_label_id_trigger() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN PERFORM update_classification_tree_tree_label_id (ARRAY_AGG(id))
FROM (
    SELECT new_classification_trees.id
    FROM new_classification_trees
...
CREATE FUNCTION public.update_dict_in_searches() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN NEW.dict = get_dict(NEW.locale);
...
CREATE FUNCTION public.upsert_ca_paths(concept_ids uuid[]) RETURNS void
    LANGUAGE plpgsql
    AS $$ BEGIN IF array_length(concept_ids, 1) > 0 THEN WITH RECURSIVE paths(
  id,
  parent_id,
  ancestor_ids,
  full_path_ids,
