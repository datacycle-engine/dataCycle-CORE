...
module DataCycleCore
  class ImportClassificationsTest < DataCycleCore::TestCases::ActiveSupportTestCase
    ...
    test 'import_classification creates the concept under its scheme' do
      concept = @subject.import_classification(
        utility_object: @utility_object,
        classification_data: { name: 'IC One', external_key: 'ic-1', tree_name: 'IC Tree' }
      )

      assert_predicate(concept, :persisted?)
      assert_equal('IC One', concept.name)
      assert_equal('ic-1', concept.external_key)
      assert_equal(@local_system.id, concept.external_system_id)

      assert_equal(concept, DataCycleCore::Concept.find_by(external_key: 'ic-1', external_system_id: @local_system.id))

      tree_label = DataCycleCore::ConceptScheme.find_by(name: 'IC Tree', external_system_id: @local_system.id)

      assert_not_nil(tree_label)
      assert_equal('IC Tree', tree_label.external_key)

      assert_equal(tree_label.id, concept.concept_scheme_id)
      assert_nil(concept.parent_concept_link.parent_id)
    end

    test 'import_classification attaches children to their parent concept' do
      parent = @subject.import_classification(
        utility_object: @utility_object,
        classification_data: { name: 'IC Parent', external_key: 'ic-parent', tree_name: 'IC Hierarchy Tree' }
      )
      child = @subject.import_classification(
        utility_object: @utility_object,
        classification_data: { name: 'IC Child', external_key: 'ic-child', tree_name: 'IC Hierarchy Tree' },
        parent:
      )

      assert_equal(parent.id, child.parent_concept_link.parent_id)
    end
    ...
    test 'import_classification updates existing classifications by external key' do
      ...
      classification = DataCycleCore::Concept.find_by(external_key: 'ic-update', external_system_id: @local_system.id)
    ...
    test 'import_classification returns nil for blank names' do
      ...
      assert_nil(DataCycleCore::Concept.find_by(external_key: 'ic-blank', external_system_id: @local_system.id))
    end
    ...
    test 'import_classification finds classifications by name without external key' do
      ...
      assert_predicate(concept, :persisted?)
      assert_nil(concept.external_key)
      assert_not_nil(DataCycleCore::Concept.find_by(name: 'IC No Key', external_system_id: @local_system.id))
    end
    ...
    test 'import_classification supports no_external_source_id option' do
      ...
      assert_nil(concept.external_system_id)
      assert_not_nil(DataCycleCore::Concept.find_by(external_key: 'ic-no-source', external_system_id: nil))
    end
    ...
    test 'import_classifications imports root classifications and their children' do
      ...
      assert_not_nil(DataCycleCore::Concept.find_by(external_key: 'icp-root', external_system_id: @local_system.id))
      assert_not_nil(DataCycleCore::Concept.find_by(external_key: 'icp-child', external_system_id: @local_system.id))
    end
    ...
    test 'import_classifications_with_filter enables filters, locale-aware extraction and respects max_count' do
      ...
      assert_not_nil(DataCycleCore::Concept.find_by(external_key: 'wf-root', external_system_id: @local_system.id))
    end
    ...
    test 'import_classifications supports filter_object iterators' do
      ...
      assert_not_nil(DataCycleCore::Concept.find_by(external_key: 'fo-root', external_system_id: @local_system.id))
    end
    ...
    test 'import_classifications skips items below min_count' do
      ...
      assert_nil(DataCycleCore::Concept.find_by(external_key: 'mc-skip', external_system_id: @local_system.id))
    end
    ...
    test 'import_classifications rescues per-item errors and fails the phase' do
      ...
      assert_nil(DataCycleCore::Concept.find_by(external_key: 'err-key', external_system_id: @local_system.id))
    end
    ...
    test 'import_classifications2 imports parents with their extracted children' do
      ...
      assert_not_nil(DataCycleCore::Concept.find_by(external_key: 'ic2-parent', external_system_id: @local_system.id))
      assert_not_nil(DataCycleCore::Concept.find_by(external_key: 'ic2-child', external_system_id: @local_system.id))
    end
    ...
    test 'import_classifications2 stops at max_count' do
      ...
      assert_not_nil(DataCycleCore::Concept.find_by(external_key: 'ic2m-a', external_system_id: @local_system.id))
      assert_nil(DataCycleCore::Concept.find_by(external_key: 'ic2m-b', external_system_id: @local_system.id))
    end
    ...
    test 'import_classification reuses an existing polygon id when updating' do
      ...
      assert_equal(1, updated_alias.concept_polygons.count)
    end
    ...
    test 'import_classifications logs partial progress every 100 items' do
      ...
      assert_not_nil(DataCycleCore::Concept.find_by(external_key: 'batch-1', external_system_id: @local_system.id))
    end
