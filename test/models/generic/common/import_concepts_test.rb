...
module DataCycleCore
  class ImportConceptsTest < DataCycleCore::TestCases::ActiveSupportTestCase
    ...
    before(:all) do
      ...
      @scheme_by_key = DataCycleCore::ConceptScheme.create!(name: 'ICT Scheme Key', external_system_id: @local_system.id, external_key: 'ict-scheme-key')
      @scheme_by_name = DataCycleCore::ConceptScheme.create!(name: 'ICT Scheme Name', external_system_id: @local_system.id)
    ...
    test 'transform_concept_scheme_identifiers groups concepts by scheme external key' do
      data_array = [{ external_key: 'c1', external_system_id: @local_system.id, name: 'C One', concept_scheme_external_key: 'ict-scheme-key', unknown_key: 'dropped' }]
      ...
      assert_equal([{ external_key: 'c1', external_system_id: @local_system.id, name: 'C One' }], result.values.first)
    end

    test 'transform_concept_scheme_identifiers resolves schemes by mapped name' do
      data_array = [{ external_key: 'c2', external_system_id: @local_system.id, name: 'C Two', concept_scheme_name: 'Raw Name' }]
      ...
      assert_equal([{ external_key: 'c2', external_system_id: @local_system.id, name: 'C Two' }], result.values.first)
    end

    test 'transform_concept_scheme_identifiers rejects missing schemes and foreign external systems' do
      data_array = [
        { external_key: 'c3', external_system_id: @local_system.id, name: 'C Three', concept_scheme_name: 'ICT Missing Scheme' },
        { external_key: 'c4', external_system_id: @remote_system.id, name: 'C Four', concept_scheme_name: 'ICT Scheme Name' }
      ]
      result = @subject.transform_concept_scheme_identifiers(data_array:, options: { import: {} })
    ...
    test 'external_system_identifiers_to_ids! resolves identifiers and names to ids' do
      ...
      assert_equal(@local_system.id, result.first[:external_system_id])
      assert_equal(@remote_system.id, result.second[:external_system_id])
    ...
    test 'external_system_identifiers_to_ids! leaves unknown identifiers without id' do
      data_array = [{ name: 'A', external_system_id: 'original-id', external_system_identifier: 'ict-unknown-system' }]
      result = @subject.external_system_identifiers_to_ids!(data_array:)

      assert_equal('original-id', result.first[:external_system_id])
    ...
    test 'transform_data_array resolves external systems and groups by scheme' do
      ...
      assert_equal(@scheme_by_key.id, result.keys.first.id)
      assert_equal([{ external_key: 'c5', external_system_id: @local_system.id, name: 'C Five' }], result.values.first)
    end

    test 'map_concept_mappings builds parent/child pairs from mapped_concepts' do
      data_array = [
        { external_key: 'p1', external_system_id: 'es-1', mapped_concepts: [{ id: 'cid-1', external_key: 'cek-1', external_system_identifier: 'esi-1', full_path: 'A > B' }] },
        { external_key: 'p2', external_system_id: 'es-1' }
      ]
      result = @subject.map_concept_mappings(data_array:, utility_object: @utility_object)
      parent = { external_key: 'p1', external_system_id: 'es-1' }

      assert_equal(2, result.size)
      assert_equal({ parent:, child: { external_key: 'cek-1', external_system_id: @utility_object.external_source.id, external_system_identifier: 'esi-1', full_path: 'A > B' } }, result.first)
      assert_equal({ parent:, child: { external_key: 'cid-1', external_system_id: @utility_object.external_source.id, full_path: 'A > B' } }, result.second)
    end

    test 'mappings_for_existing_concepts resolves concepts by external key and by full path' do
      concept_mappings = [
        { parent: { external_key: 'ict-parent', external_system_id: @local_system.id }, child: { external_key: 'ict-child', external_system_id: @local_system.id, full_path: 'Unknown > Path' } },
        { parent: { external_key: 'ict-parent', external_system_id: @local_system.id }, child: { external_key: 'ict-missing', external_system_id: @local_system.id, full_path: 'ICT Mapping Tree > ICT Child' } }
      ]
    ...
    test 'mappings_for_existing_concepts drops unresolvable mappings' do
      concept_mappings = [
        { parent: { external_key: 'ict-unknown-parent', external_system_id: @local_system.id }, child: { external_key: 'ict-child', external_system_id: @local_system.id, full_path: 'Unknown > Path' } }
      ]
    ...
    test 'transform_concept_mappings resolves mapped concepts to concept links' do
      data_array = [{ external_key: 'ict-parent', external_system_id: @local_system.id, mapped_concepts: [{ id: 'ict-child', full_path: 'Unknown > Path' }] }]
    ...
    test 'transform_geometries maps geometries to classification alias ids' do
      data_array = [
        { external_system_id: @local_system.id, external_key: 'ict-child', geom: 'MULTIPOLYGON (((1 1, 2 2, 3 3, 1 1)))' },
        { external_system_id: @local_system.id, external_key: 'ict-parent', geom: BSON::Binary.new('wkb-bytes') },
        { external_system_id: @local_system.id, external_key: 'ict-unknown', geom: 'POINT (1 2)' },
        { external_system_id: @local_system.id, external_key: 'ict-no-geom' }
      ]
      result = @subject.transform_geometries(data_array:)

      assert_equal(2, result.size)
      assert_includes(result, { concept_id: @child_alias.id, geom: 'MULTIPOLYGON (((1 1, 2 2, 3 3, 1 1)))' })
      assert_includes(result, { concept_id: @parent_alias.id, geom: 'wkb-bytes' })
    end
