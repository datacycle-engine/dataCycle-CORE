...
module DataCycleCore
  module Generic
    class FeratelIdentityServerTest < ActiveSupport::TestCase
      def setup
        @cw_temp = DataCycleCore::Thing.where(template: false).count
      end

      def download_from_local_json(external_source)
        path = Rails.root.join('..', 'fixtures', 'external_sources', 'feratel_indentity_server')
        files = path + '*.json'
      ...
      test 'perform import' do
        ...
        external_source = DataCycleCore::ExternalSystem.find_by(name: 'Feratel Identity Server')
        download_from_local_json(external_source)
        external_source.import(options)

        assert_equal(1, DataCycleCore::Thing.where(template: false).with_schema_type('Organization').count)
      end

      def teardown
        DataCycleCore::MongoHelper.drop_mongo_db('Feratel Identity Server')
      end
    end
  end
end
