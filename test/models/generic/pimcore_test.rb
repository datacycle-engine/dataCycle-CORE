  ...
  module Generic
    class PimcoreTest < ActiveSupport::TestCase
      ...
      def download_from_local_json(external_source)
        path = Rails.root.join('..', 'fixtures', 'external_sources', 'pimcore')
        files = path + '*.json'
      ...
      test 'perform import' do
        ...
        external_source = DataCycleCore::ExternalSystem.find_by(name: 'Pimcore')
        ...
        assert_equal(1, DataCycleCore::ClassificationAlias.for_tree('Pimcore - Cities').count)
        assert_equal(1, DataCycleCore::ClassificationAlias.for_tree('Pimcore - Categories').count)
