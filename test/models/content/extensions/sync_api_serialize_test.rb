...
module DataCycleCore
  class SyncApiSerializeTest < ActiveSupport::TestCase
    ...
    test 'event with overlay' do
      ...
      assert_equal(event.name_overlay, main_data['name'])
      assert_equal(event.description_overlay, main_data['description'])
    end
    ...
    test 'event_with_overlay_schedule' do
      ...
      assert_nil(main_data['event_date'])
    end
    ...
    test 'event_with_event_schedule' do
      ...
      serialized_schedule = main_data.dig('event_schedule', 0)

      assert_not(serialized_schedule.key?('thing_id') || serialized_schedule.key?(:thing_id))
      assert_not(serialized_schedule.key?('relation') || serialized_schedule.key?(:relation))
    end
    ...
    test 'event with linked image' do
      ...
      assert_equal(['image'], included_image['attribute_name'])
      assert_nil(included_image['included'])

      included_image_data = included_image['de']

      assert_equal(overlay_image.id, included_image_data['id'])
      ['id', 'name', 'upload_date', 'template_name', 'updated_at', 'created_at'].each do |attribute|
        assert_predicate(included_image_data[attribute], :present?)
      end
    end
    ...
    def create_event_new_overlay
      DataCycleCore::TestPreparations.create_content(template_name: 'EventNewOverlay', data_hash: { name: 'Test Event' })
    end

    test 'EventNewOverlay name_override is serialized as name' do
      event = create_event_new_overlay
      update_event(event, { name_override: 'Overridden Name' })
      ...
      ['id', 'name', 'template_name', 'updated_at', 'created_at'].each do |attribute|
        assert_predicate(main_data[attribute], :present?)
      end

      assert_equal('Overridden Name', event.name_overlay)
      assert_equal('Overridden Name', main_data['name'])
    end

    test 'EventNewOverlay image_override is serialized as image' do
      ...
      event = create_event_new_overlay
      update_event(event, { image: [image.id], image_override: [override_image.id] })
      ...
      assert_equal([override_image.id], event.image_overlay.pluck(:id))
      assert_equal(override_image.id, main_data.dig('image', 0))

      included_image = serialized_event['included'].detect { |i| i.dig('de', 'id') == override_image.id }

      assert_not_nil(included_image)
      assert_equal(['image'], included_image['attribute_name'])
    end

    test 'EventNewOverlay name_override and image_override together are serialized as name and image' do
      image = DataCycleCore::TestPreparations.create_content(template_name: 'Bild', data_hash: { name: 'Original Image' })
      override_image = DataCycleCore::DummyDataHelper.create_data('image')
      ...
      update_event(event, {
        ...
        image_override: [override_image.id]
      })
