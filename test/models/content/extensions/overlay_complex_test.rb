...
module DataCycleCore
  class OverlayComplexTest < ActiveSupport::TestCase
    ...
    test 'event with proper schedule and computed properties' do
      ...
      assert_equal(dtfrom, event.start_date.to_fs(:long_datetime))
      assert_equal(dtend, event.end_date.to_fs(:long_datetime))
      assert_equal(odtfrom, event.overlay.first.start_date.to_fs(:long_datetime))
      assert_equal(odtend, event.overlay.first.end_date.to_fs(:long_datetime))
      assert_equal(odtfrom, event.start_date_overlay.to_fs(:long_datetime))
      assert_equal(odtend, event.end_date_overlay.to_fs(:long_datetime))
    end
  end
end
