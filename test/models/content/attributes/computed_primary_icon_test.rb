      ...
      class ComputedPrimaryIconTest < DataCycleCore::TestCases::ActiveSupportTestCase
        ...
        test 'override wins over the assigned classifications' do
          with_icons do
            content = create_content(
              'universal_classifications' => [@tags['Tag 2'].id],
              'primary_icon_tags' => [@tags['Tag 3'].id]
            )

            assert_equal([@tags['Tag 3'].id], primary_icon_ids(content))
          end
        end
        ...
        test 'override on a child concept resolves to the nearest ancestor with an icon' do
          with_icons do
            ...
            content = create_content('primary_icon_tags' => [@tags['Nested Tag 1'].id])
        ...
        test 'override without any icon in its ancestry yields no primary icon' do
          with_icons do
            content = create_content('primary_icon_tags' => [@tags['Tag 1'].id])
        ...
        test 'without override the first assigned classification with icon wins (tree order)' do
          with_icons do
            content = create_content(
              'universal_classifications' => [@tags['Tag 3'].id, @tags['Tag 2'].id, @markets['Markt 1'].id]
            )

            # Tag 2 comes before Tag 3 in tree order, Markt 1 has no icon
            assert_equal([@tags['Tag 2'].id], primary_icon_ids(content))
          end
        end
        ...
        test 'fallback resolves an assigned leaf concept to its icon-bearing ancestor' do
          with_icons do
            ...
            content = create_content('universal_classifications' => [@tags['Nested Tag 1'].id])
        ...
        test 'override and fallback merge across both trees' do
          with_icons do
            content = create_content(
              'primary_icon_tags' => [@tags['Tag 2'].id],
              'universal_classifications' => [@markets['Markt 2'].id]
            )

            assert_equal(
              Set[@tags['Tag 2'].id, @markets['Markt 2'].id],
        ...
        test 'removing override and classifications clears the computed value' do
          with_icons do
            content = create_content('primary_icon_tags' => [@tags['Tag 2'].id])
        ...
        test 'within a tree the first non-blank override wins (parameter order = priority)' do
          with_icons do
            ...
            content = create_content(
              'primary_icon_tags' => [@tags['Tag 2'].id],
              'primary_icon_tags_secondary' => [@tags['Tag 3'].id]
            )
        ...
        test 'within a tree the secondary override is used when the first is blank' do
          with_icons do
            content = create_content('primary_icon_tags_secondary' => [@tags['Tag 3'].id])
        ...
        test 'a hidden mapping does not drive the primary icon (#47172/#47053/#50677)' do
          with_icons do
            ...
            source_tree = DataCycleCore::ConceptScheme.create!(name: "IconSrc_#{SecureRandom.hex(6)}")
            source_concept = source_tree.create_concept('MappedSource')
            DataCycleCore::ConceptLink.create!(parent: @tags['Tag 2'], child: source_concept, link_type: DataCycleCore::ConceptLink::LINK_TYPE_RELATED)
            tags_tree = @tags['Tag 2'].concept_scheme
            tags_tree.update!(hidden_mappings: true)

            content = create_content('universal_classifications' => [source_concept.id])
        ...
        test 'changing classifications through the save path recomputes the value' do
          with_icons do
            content = create_content('universal_classifications' => [@tags['Tag 2'].id])

            assert_equal([@tags['Tag 2'].id], primary_icon_ids(content))

            content.set_data_hash(data_hash: { 'universal_classifications' => [@tags['Tag 3'].id] })

            assert_equal([@tags['Tag 3'].id], primary_icon_ids(content), 'Tag 2 was removed, so it must not resurrect from stale relations')
          end
        end
        ...
        test 'the compute observes the persisted classifications, not the pre-save state' do
          with_icons do
            # the fallback reads collected_concept_contents, which its triggers only fill
            # once the concept_contents rows are written — an inline before_save compute
        ...
        test 'the recompute keeps the acting user as updated_by' do
          with_icons do
            ...
            content.set_data_hash(data_hash: { 'universal_classifications' => [@tags['Tag 3'].id] }, current_user: user)

            reloaded = DataCycleCore::Thing.find(content.id)

            assert_equal([@tags['Tag 3'].id], Array.wrap(reloaded.get_data_hash['primary_icon_classifications']))
          ...
          with_icons do
            ...
            content.stub(:update_after_save_computed_values, ->(*) { flunk('update_computed: false must not recompute') }) do
              content.set_data_hash(
                data_hash: { 'universal_classifications' => [@tags['Tag 3'].id] },
                update_computed: false
              )
            end

            assert_equal([@tags['Tag 2'].id], primary_icon_ids(content), 'the caller opted out, so the stored icon stays as it was')
          end
        end
        ...
        test 'an icon-neutral classification change does not report "no changes"' do
          with_icons do
            ...
            content.set_data_hash(
              data_hash: { 'universal_classifications' => [@tags['Tag 2'].id, @markets['Markt 1'].id] }
            )
