    ...
    module Attributes
      ...
      class ComputedOverrideOrMappedTest < DataCycleCore::TestCases::ActiveSupportTestCase
        before(:all) do
          ...
          @source_tree = DataCycleCore::ConceptScheme.create!(name: "OomSrc_#{SecureRandom.hex(6)}")
          @source_concept = @source_tree.create_concept('MappedSource')
          @mapping = DataCycleCore::ConceptLink.create!(parent: @tags['Tag 1'], child: @source_concept, link_type: DataCycleCore::ConceptLink::LINK_TYPE_RELATED)
        end
        ...
        test 'without override the effective classification is the mapped target concept' do
          ...
          assert_equal [@tags['Tag 1'].id], effective_ids(content)
        end

        test 'the override wins over the mapped classification' do
          content = create_content(
            'universal_classifications' => [@source_concept.id],
            'override_tag' => [@tags['Tag 2'].id]
          )

          assert_equal [@tags['Tag 2'].id], effective_ids(content)
        end
        ...
        test 'the effective classification is stored by the save that triggers it' do
          ...
          assert_equal [@tags['Tag 1'].id], Array.wrap(content.get_data_hash['effective_tag'])
        end

        test 'a later save updates the effective classification without a recompute job' do
          content = create_content('universal_classifications' => [@source_concept.id])

          content.set_data_hash(data_hash: { 'override_tag' => [@tags['Tag 2'].id] })
        ...
        test 'the mapped classification is resolved even when the target tree hides mappings (#50677)' do
          tags_tree = @tags['Tag 1'].concept_scheme
