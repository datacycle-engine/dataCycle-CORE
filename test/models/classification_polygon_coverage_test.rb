...
module DataCycleCore
  # Coverage for ConceptPolygon's spatial class-methods. The read aggregates
  ...
  class ClassificationPolygonCoverageTest < DataCycleCore::TestCases::ActiveSupportTestCase
    test 'to_bbox builds and runs the extent json query' do
      assert_nothing_raised { DataCycleCore::ConceptPolygon.to_bbox }
    end
    ...
    test 'to_mvt builds and runs the MVT tile query (named and default layer)' do
      ...
      relation = DataCycleCore::ConceptPolygon
        .joins(:concept)
        .where(concept: { id: [SecureRandom.uuid] })
    ...
    test 'combined_geojson builds and runs the union geojson query' do
      assert_nothing_raised { DataCycleCore::ConceptPolygon.combined_geojson }
    end

    test 'upsert_all_geoms returns 0 for blank data and inserts the slices otherwise' do
      assert_equal 0, DataCycleCore::ConceptPolygon.upsert_all_geoms([])
      assert_equal 0, DataCycleCore::ConceptPolygon.upsert_all_geoms(nil)

      alias_id = DataCycleCore::Concept.first.id
      factory = RGeo::Geographic.spherical_factory(srid: 4326)
      geom = factory.point(11.0, 47.0)

      # geom_simple is a generated column - only geom is inserted.
      count = DataCycleCore::ConceptPolygon.upsert_all_geoms(
        [{ concept_id: alias_id, geom: }]
      )
