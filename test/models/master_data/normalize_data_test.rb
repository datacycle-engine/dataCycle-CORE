...
describe DataCycleCore::MasterData::NormalizeData do
  ...
  describe 'hash_helper' do
    ...
    it 'updates a value deep in the data_hash' do
      ...
      assert_equal(6, hash['a'])
      hash = subject.update_path(hash1, 'c/d', 6)

      assert_equal(6, hash.dig('c', 'd'))
      hash = subject.update_path(hash1, 'c/e/f', 6)

      assert_equal(6, hash.dig('c', 'e', 'f'))
    end
  ...
  describe 'normalize_data' do
    ...
    it 'grabs correctly all normalizable data attributs' do
      assert_equal(normalizable_hash, subject.normalizable_data(nil, person_template['properties'], data_hash))
    end
    ...
    it 'does the preprocessing of the data correctly' do
      ...
      assert_equal(normalizable_data, norm_hash)
      assert_equal(transformation_hash, transformation)
    end

    it 'merges correctly street and street_nr' do
      merged_report = subject.merge_street_streetnr(normalize_report)

      assert_equal(merged_fields, merged_report.dig('entry', 'fields'))
      new_action_list = normalize_report['actionList'].deep_dup

      assert_equal(new_action_list, merged_report['actionList'])
    end
    ...
    it 'back_transforms normalized_data to original ids and schema' do
      ...
      assert_equal(back_transformed_data, back_trans)
    end

    it 'correctly updates data according to normalized action_list' do
      assert_equal(returned_data, subject.update_data(data_hash, back_transformed_data))
    end
    ...
    it 'does post_processing correctly' do
      ...
      assert_equal(diff_hash, diffs)
      assert_equal(returned_data, updated_data)
    end

    it 'does the whole normalization correctly' do
      logger = DataCycleCore::Generic::Logger::LogFile.new('normalize')
      normalizer = subject.new(logger:)
      endpoint_mock = Minitest::Mock.new
      endpoint_mock.expect(:normalize, normalize_report) do |_id, data_list, _comment|
        assert_equal(normalizable_data, data_list)
        true
      end
      normalizer.endpoint = endpoint_mock

      updated_data, diffs = normalizer.normalize(data_hash, person_template)
    ...
    it 'returns original data and a empty hash if no template, or no data a given' do
      ...
      assert_equal({}, diffs)
      assert_equal(data_hash, updated_data)
