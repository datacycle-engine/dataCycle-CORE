...
module DataCycleCore
  ...
  class ConceptRecomputeTest < DataCycleCore::TestCases::ActiveSupportTestCase
    before(:all) do
    ...
    def create_content(source_concept)
      DataCycleCore::TestPreparations.create_content(
        ...
        data_hash: { 'name' => "Recompute #{source_concept.id}", 'universal_classifications' => [source_concept.id] }
      )
    end
    ...
    def create_parent_tag_content(concept)
      DataCycleCore::TestPreparations.create_content(
        ...
        data_hash: { 'name' => "Below #{concept.name}", 'universal_classifications' => [concept.id] }
      )
    end
    ...
    def rename_concept(concept, name)
      ActionCable.server.stub(:broadcast, ->(*) {}) do
        perform_enqueued_jobs do
          DataCycleCore::Concept.find(concept.id).update!(name:)
        end
      end
    end
    ...
    def move_concept(concept, new_parent)
      ActionCable.server.stub(:broadcast, ->(*) {}) do
        perform_enqueued_jobs do
          alias_record = DataCycleCore::Concept.find(concept.id)
          alias_record.move_to_scheme(new_parent.id, alias_record.concept_scheme.id)
        end
      end
    end
    ...
    # A move stales the same values and writes no content either, and it lands on concept_links,
    # so no after_update on the concept fires — only move_to_scheme can enqueue this.
    ...
    test 'moving a concept into a tree with an opted-in property enqueues the recompute' do
      ...
      alias_record = DataCycleCore::Concept.find(@source1.id)
      alias_record.concept_scheme # memoize the old tree before the move

      calls = capture_destroy_jobs do
        ActionCable.server.stub(:broadcast, ->(*) {}) do
          alias_record.move_to_scheme(@tag1.id, DataCycleCore::ConceptScheme.find_by(name: 'Tags').id)
        end
      end
    ...
    test 'adding a mapping recomputes the affected contents' do
      with_icons do
        ...
        apply_mapping(@tag3.id, insert: [@source1.id])

        assert_equal([@tag3.id], stored_icon_ids(content))
      end
    end
    ...
    test 'removing a mapping recomputes contents whose stored icon sits on an ancestor of the leaf' do
      with_icons do
        ...
        apply_mapping(@nested_tag.id, insert: [@source1.id])
        ...
        apply_mapping(@nested_tag.id, delete: [@source1.id])
    ...
    test 'a multi-classification mapping delta recomputes every affected content (union)' do
      with_icons do
        ...
        apply_mapping(@tag3.id, insert: [@source1.id, @source2.id])

        assert_equal([@tag3.id], stored_icon_ids(content1))
        assert_equal([@tag3.id], stored_icon_ids(content2))
      end
    end
    ...
    test 'a change on an alias in an unrelated tree does not recompute the contents' do
      with_icons do
        content = create_content(@source1)
        source_alias = DataCycleCore::Concept.find(@source1.id)
