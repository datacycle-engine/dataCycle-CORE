...
module DataCycleCore
  class ScheduleSearchTest < DataCycleCore::TestCases::ActiveSupportTestCase
    ...
    test 'test query for event_schedule with single occurrence' do
      ...
      items = DataCycleCore::Filter::Search.new(locale: :de).schedule_search({ 'from' => Date.current, 'until' => Date.current })
    ...
    test 'test query for event_schedule should interpret date without time as start of day or end of day' do
      create_content('Event', { name: 'DDD0', event_schedule: schedule_hash(Time.zone.now.beginning_of_day + 1.hour) })

      items = DataCycleCore::Filter::Search.new(locale: :de).schedule_search({ 'until' => Date.current.iso8601 })

      assert_equal(1, items.count)

      items = DataCycleCore::Filter::Search.new(locale: :de).schedule_search({ 'until' => Time.zone.now.beginning_of_day.iso8601 })

      assert_equal(0, items.count)

      items = DataCycleCore::Filter::Search.new(locale: :de).schedule_search({ 'from' => Date.current.iso8601 })

      assert_equal(1, items.count)

      items = DataCycleCore::Filter::Search.new(locale: :de).schedule_search({ 'from' => Time.zone.now.end_of_day.iso8601 })

      assert_equal(0, items.count)
    end

    test 'test query for event_schedule should not find events at the end of their schedule' do
      create_content('Event', { name: 'DDD0', event_schedule: schedule_hash(Time.zone.now.beginning_of_day + 1.hour) })

      items = DataCycleCore::Filter::Search.new(locale: :de).schedule_search({ 'from' => (Time.zone.now.beginning_of_day + 2.hours).iso8601, 'until' => (Time.zone.now.beginning_of_day + 2.hours).iso8601 })
