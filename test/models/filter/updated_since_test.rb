  ...
  module Filter
    class UpdatedSinceTest < ActiveSupport::TestCase
      ...
      test 'sanity check for updated_since' do
        assert_equal(6, DataCycleCore::Thing.where("things.updated_at >= '#{@timestamp.utc}'").count)
        assert_equal(0, DataCycleCore::Thing.where("things.updated_at > '#{Time.zone.now.utc}'").count)
