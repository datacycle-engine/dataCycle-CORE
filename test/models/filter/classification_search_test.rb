...
module DataCycleCore
  class ClassificationSearchTest < DataCycleCore::TestCases::ActiveSupportTestCase
    before(:all) do
      ...
      create_content('Artikel', { name: 'HEADLINE 3', tags: get_concept_ids('Tags', ['Tag 2', 'Tag 3']) })
      create_content('Artikel', { name: 'HEADLINE 4', tags: get_concept_ids('Tags', ['Tag 2']) })
    end

    test 'filter contents based on directly assigned classifications by id' do
      items = DataCycleCore::Filter::Search.new(locale: :de)
        .concept_ids_without_subtree(get_concept_ids('Tags', 'Tag 3'))

      assert_equal(2, items.count)

      items = DataCycleCore::Filter::Search.new(locale: :de)
        .concept_ids_without_subtree(get_concept_ids('Tags', 'Tag 2'))
    ...
    test 'filter contents based on classification hierarchy by id' do
      items = DataCycleCore::Filter::Search.new(locale: :de)
        .concept_ids_with_subtree(get_concept_ids('Tags', 'Tag 3'))
      ...
      items = DataCycleCore::Thing
        .with_concept_ids(get_concept_ids('Tags', 'Tag 3'))
    ...
    test 'filter contents by excluding classifications by id' do
      items = DataCycleCore::Filter::Search.new(locale: :de)
        .not_concept_ids_with_subtree(get_concept_ids('Tags', 'Tag 2'))

      assert_equal(5, items.count)
    end

    test 'filter contents after updating classifications' do
      article1 = create_content('Artikel', { name: 'ARTICLE 1', tags: get_concept_ids('Tags', ['Tag 1']) })
      article2 = create_content('Artikel', { name: 'ARTICLE 2', tags: get_concept_ids('Tags', ['Tag 1', 'Tag 2']) })
      article3 = create_content('Artikel', { name: 'ARTICLE 3', tags: get_concept_ids('Tags', ['Tag 1', 'Tag 3']) })
      ...
      update_content(article2, { tags: get_concept_ids('Tags', ['Tag 2']) })
      update_content(article3, { tags: get_concept_ids('Tags', ['Tag 3']) })
