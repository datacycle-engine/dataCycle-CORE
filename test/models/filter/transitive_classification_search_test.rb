...
module DataCycleCore
  class TransitiveClassificationSearchTest < DataCycleCore::TestCases::ActiveSupportTestCase
    before(:all) do
      ...
      @content = DataCycleCore::TestPreparations.create_content(template_name: 'Artikel', data_hash: { name: 'TEST 1 ARTIKEL', tags: DataCycleCore::Concept.ids_for_tree_with_name(@tags.name, 'Tag 1') })

      @tree2 = DataCycleCore::ConceptScheme.create!(name: 'Tree 2')
      @tree3 = DataCycleCore::ConceptScheme.create!(name: 'Tree 3')
      @dummy_tree = DataCycleCore::ConceptScheme.create!(name: 'Tree Dummy')
      @tree2.create_concept('parent 1', 'mapped 1')
      @tree2.create_concept('parent 1', 'mapped 1.1')
      @tree3.create_concept('parent 2', 'mapped 2')
      @dummy_tree.create_concept('parent dummy', 'mapped dummy')

      tag1 = DataCycleCore::Concept.for_tree(@tags.name).find_by!(internal_name: 'Tag 1')
      tag2 = DataCycleCore::Concept.for_tree(@tags.name).find_by!(internal_name: 'Tag 2')
      mapped1 = DataCycleCore::Concept.for_tree(@tree2.name).find_by!(internal_name: 'mapped 1')
      @mapped12 = DataCycleCore::Concept.for_tree(@tree2.name).find_by!(internal_name: 'mapped 1.1')
      ...
      dummy = DataCycleCore::Concept.for_tree(@dummy_tree.name).find_by!(internal_name: 'mapped dummy')

      mapped1.update!(mapped_concept_ids: [tag1.id, dummy.id])
      @mapped12.update!(mapped_concept_ids: [tag2.id])
      mapped2.update!(mapped_concept_ids: [parent1.id])
    end
    ...
    test 'recursion in mappings works without infinite loop' do
      ...
      ActiveRecord::Base.transaction(joinable: false, requires_new: true) do
        ...
        Timeout.timeout(timeout) do
          ...
          assert_equal([mapped2.id], mapped1.reload.mapped_concept_ids)
          mapped2.update!(mapped_concept_ids: [mapped1.id])

          assert_equal([mapped1.id], mapped2.reload.mapped_concept_ids)
        end
      end
    end

    test 'filter contents based on mapped (2 hops) classifications by id' do
      items = DataCycleCore::Filter::Search.new(locale: :de).concept_ids_with_subtree(
        DataCycleCore::Concept.for_tree(@tree3.name).with_internal_name('mapped 2').pluck(:id)
      )
    ...
    test 'filter contents based on mapped (1 hop) classifications by id' do
      items = DataCycleCore::Filter::Search.new(locale: :de).concept_ids_with_subtree(
        DataCycleCore::Concept.for_tree(@tree2.name).with_internal_name('mapped 1').pluck(:id)
      )
    ...
    test 'filter contents based on mapped (2 hops) classifications by id without subtree' do
      items = DataCycleCore::Filter::Search.new(locale: :de).concept_ids_without_subtree(
        DataCycleCore::Concept.for_tree(@tree3.name).with_internal_name('mapped 2').pluck(:id)
      )
    ...
    test 'filter contents based on mapped (1 hop) classifications by id without subtree' do
      ...
      items = items.concept_ids_without_subtree(
        DataCycleCore::Concept.for_tree(@tree2.name).with_internal_name('mapped 1').pluck(:id)
      )
    ...
    test 'filter contents based on mapped (2 hops) classifications, excluding contents with target classification' do
      items = DataCycleCore::Filter::Search.new(locale: :de).concept_ids_with_subtree(
        ...
        DataCycleCore::Concept.for_tree(@tags.name).with_internal_name('Tag 1').pluck(:id)
      )
    ...
    test 'filter contents based on mapped (1 hop) parent classifications by id' do
      items = DataCycleCore::Filter::Search.new(locale: :de).concept_ids_with_subtree(
        DataCycleCore::Concept.for_tree(@tree2.name).with_internal_name('parent 1').pluck(:id)
      )
    ...
    test 'filter contents based on mapped (2 hops) parent classifications by id' do
      items = DataCycleCore::Filter::Search.new(locale: :de).concept_ids_with_subtree(
        DataCycleCore::Concept.for_tree(@tree3.name).with_internal_name('parent 2').pluck(:id)
      )
    ...
    test 'filter contents based on mapped (1 hop) parent classifications by id after removing 1 mapping' do
      @mapped12.update!(mapped_concept_ids: [])
