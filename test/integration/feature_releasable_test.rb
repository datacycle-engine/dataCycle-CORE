...
module DataCycleCore
  class FeatureReleasableTest < ActionDispatch::IntegrationTest
    ...
    test 'change release status when creating external link' do
      ...
      follow_redirect!
      assert_equal DataCycleCore::Feature::Releasable.get_stage('partner'), @content.try(DataCycleCore::Feature::Releasable.allowed_attribute_keys(@content)&.first)&.reload&.first&.name
    end
    ...
    test 'change release status after finished editing content' do
      ...
      assert_equal DataCycleCore::Feature::Releasable.get_stage('review'), @content.try(DataCycleCore::Feature::Releasable.allowed_attribute_keys(@content)&.first)&.reload&.first&.name
    end
  end
end
