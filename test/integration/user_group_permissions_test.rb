...
module DataCycleCore
  class UserGroupPermissionsTest < DataCycleCore::TestCases::ActiveSupportTestCase
    ...
    test 'user_group permissions, with collection, no template' do
      ...
      assert_not @permissions_test_user.can?(:merge_duplicates, @dummy_poi1)
    ...
    test 'user_group permissions, with collection, with template' do
      ...
      assert_not @permissions_test_user.can?(:merge_duplicates, @dummy_poi1)
      assert @permissions_test_user.can?(:set_life_cycle, @dummy_poi1)
      assert_not @permissions_test_user.can?(:set_life_cycle, @dummy_event)
    end

    test 'user_group permissions, no collection, with template' do
      reset_permission_test_user_groups
      update_permissions_test_user_group(@test_group3, {user_ids: [@permissions_test_user.id]})

      assert @permissions_test_user.can?(:set_life_cycle, @dummy_poi1)
      assert @permissions_test_user.can?(:set_life_cycle, @dummy_poi2)
      assert_not @permissions_test_user.can?(:set_life_cycle, @dummy_event)
    end
    ...
    def reset_permission_test_user_groups
      ...
      update_permissions_test_user_group(@test_group3, {user_ids: []})
    end
