          ...
          module Events
            class Event < DataCycleCore::TestCases::ActionDispatchIntegrationTest
              ...
              test 'APIv2 json equals APIv3 json result' do
                ...
                assert_equal(1, api_v3_json['eventSchedule'].size)
                assert_equal(1, api_v2_json['eventSchedule'].size)
                assert_equal(convert_api_v2_json['subEvent'].map { |item| item.except(*except_sub_event_params) }, v2_subevents.map { |item| item.except(*except_sub_event_params) })
                assert_equal(convert_api_v3_json['subEvent'].map { |item| item.except(*except_sub_event_params) }, v3_subevents.map { |item| item.except(*except_sub_event_params) })
                assert_equal(api_v3_json['image'].first.except(*except_sub_event_params), api_v2_json['image'].first.except(*except_sub_event_params))
              end
            end
          end
        end
