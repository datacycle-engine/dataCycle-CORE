      ...
      module Content
        class IncludeTest < ActionDispatch::IntegrationTest
          ...
          test 'tour at /api/v4/things/:id serializes without included embedded/linked data' do
            ...
            (@content_tour.embedded_property_names + @content_tour.linked_property_names - ['overlay', 'subject_of', 'is_linked_to', 'linked_thing', 'potential_action']).each do |embedded|
          ...
          test 'tour with included embedded schedule' do
            ...
            (@content_tour.embedded_property_names + @content_tour.linked_property_names - ['overlay', 'schedule', 'subject_of', 'is_linked_to', 'linked_thing', 'potential_action']).each do |embedded|
              json_key = @content_tour.schema.dig('properties', embedded, 'api', 'v4', 'name') || @content_tour.schema.dig('properties', embedded, 'api', 'name') || embedded.camelize(:lower)
              assert_compact_header(json_data.dig(json_key))
            end
          ...
          test 'tour with included linked poi' do
            ...
            # all embedded/linked have a compact header
            (@content_tour.embedded_property_names + @content_tour.linked_property_names - ['overlay', 'poi', 'subject_of', 'is_linked_to', 'linked_thing', 'potential_action']).each do |embedded|
              json_key = @content_tour.schema.dig('properties', embedded, 'api', 'v4', 'name') || @content_tour.schema.dig('properties', embedded, 'api', 'name') || embedded.camelize(:lower)
              assert_compact_header(json_data.dig(json_key))
            end
          ...
          test 'tour with multiple includes' do
            ...
            # all embedded/linked have a compact header
            (@content_tour.embedded_property_names + @content_tour.linked_property_names - ['overlay', 'poi', 'image', 'subject_of', 'is_linked_to', 'linked_thing', 'potential_action']).each do |embedded|
              json_key = @content_tour.schema.dig('properties', embedded, 'api', 'v4', 'name') || @content_tour.schema.dig('properties', embedded, 'api', 'name') || embedded.camelize(:lower)
              assert_compact_header(json_data.dig(json_key))
            end
