...
module DataCycleCore
  module Api
    module V4
      module General
        module Links
          class ConceptsTest < DataCycleCore::TestCases::ActionDispatchIntegrationTest
            include DataCycleCore::ApiV4Helper

            before(:all) do
              @routes = Engine.routes
              @user = User.find_by(email: 'tester@datacycle.at')
              @test_route = method(:classifications_api_v4_concept_scheme_path)
              @concept_scheme = DataCycleCore::ConceptScheme.find_by(name: 'Gravity')
              @total = @concept_scheme.concepts.size
            ...
            test 'GET concept_schemes full all sections' do
              ...
              assert_predicate(json_data['@graph'], :present?)
            ...
            def test_route(params = {})
              @test_route.call(params.merge(id: @concept_scheme.id))
            end
          end
        end
      end
