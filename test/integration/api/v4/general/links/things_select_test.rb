...
module DataCycleCore
  module Api
    module V4
      module General
        module Links
          class ThingsSelectTest < DataCycleCore::TestCases::ActionDispatchIntegrationTest
            ...
            before(:all) do
              ...
              @test_route = method(:api_v4_contents_select_path)
              DataCycleCore::DummyDataHelper.create_data('poi')
              DataCycleCore::DummyDataHelper.create_data('event')

              @ids = DataCycleCore::Thing.where.not(content_type: 'embedded').pluck(:id).join(',')
            ...
            def test_route(params = {})
              @test_route.call(params.merge(uuids: @ids))
            end
