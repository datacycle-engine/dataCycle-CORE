...
module DataCycleCore
  ...
  self.classification_visibilities = ['show', 'api', 'tile', 'show_more', 'xml', 'compact', 'edit', 'list', 'filter', 'classification_overview', 'classification_administration', 'tree_view', 'content_classifier']
  ...
  self.header_title = -> { I18n.t("environment_name.#{Rails.env}", default: Rails.env) unless Rails.env.production? }
  ...
  # shared ActiveSupport deprecator for the engine; the class-level ActiveSupport::Deprecation.warn
  # is no longer available in Rails 8, so use this instance (e.g. DataCycleCore.deprecator.warn(...))
  def self.deprecator
    @deprecator ||= ActiveSupport::Deprecation.new('a future release', 'DataCycleCore')
  end
  ...
  def self.default_classification_visibilities
    classification_visibilities.except(['show_more', 'compact', 'tree_view', 'classification_overview'])
  end
  ...
  class Engine < ::Rails::Engine
    ...
    config.before_initialize do |app|
      # resolve *_FILE=/run/secrets/* env vars into their plain counterparts
      # mirrors the behaviour of init/resolve-secrets.sh for docker exec / rake invocations
      ENV.select { |k, v| k.end_with?('_FILE') && v.start_with?('/run/secrets/') }.each do |file_var, path|
        var = file_var.delete_suffix('_FILE')
        next if ENV.key?(var) && !ENV[var].empty?
        raise "Secret file #{path} for environment variable '#{var}' does not exist or is not a regular file" unless File.file?(path)
      ...
      app.middleware.insert 0, DataCycleCore::RejectEncodedPathTraversalMiddleware
