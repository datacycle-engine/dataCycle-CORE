...
namespace :dc do
  namespace :import do
    ...
    task :append_job, [:external_source_name] => [:environment] do |_, args|
      ...
      DataCycleCore::ImportJob.perform_later(external_source.id) unless Delayed::Job.exists?(queue: 'importers', delayed_reference_type: 'import', delayed_reference_id: external_source.id, locked_at: nil)
    end
  end
end
