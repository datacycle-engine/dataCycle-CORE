...
namespace :dc do
  namespace :refactor do
    ...
    task :add_dummy, [:overlay_name] => [:environment] do |_, args|
      ...
      abort('ERROR: template does not exist, or is not of type embedded') if DataCycleCore::Thing.new(template_name: overlay_name).embedded?
      overlays = DataCycleCore::Thing.where(template_name: overlay_name)
      overlays.each do |i|
