  ...
  class ImportJob < ApplicationJob
    ...
    def perform(uuid)
      ...
      pid = Process.fork do
        ...
        Appsignal.send_error(exception, nil, "download import job failed - #{external_source.id}")
        external_source.config['last_download_import_failed'] = true
        external_source.config['last_download_import_exception'] = "#{exception} (#{Time.zone.now})"
        external_source.save!
      end
      Process.waitpid(pid)
