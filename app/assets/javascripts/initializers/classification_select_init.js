...
export default function () {
  ...
  DataCycle.initNewElements('.classification-checkbox-list:not(.dc-check-box-selector)', e =>
    new CheckBoxSelector(e).init()
  );

  DataCycle.initNewElements('.classification-radiobutton-list:not(.dc-radio-button-selector)', e =>
    new RadioButtonSelector(e).init()
  );

  DataCycle.initNewElements('.auto-tagging-button:not(.dcjs-auto-tagging)', initAutoTagging.bind(this));
  DataCycle.initNewElements('.async-select:not(.dcjs-select2)', e => new AsyncSelect2(e).init());
  DataCycle.initNewElements('.single-select:not(.dcjs-select2), .multi-select:not(.dcjs-select2)', e =>
    new SimpleSelect2(e).init()
  );
}
