...
class ImageEditor {
	static selector = ".image-editor-reveal";
	...
	setFileFormat(mimeType) {
		...
		this.updateOuterEditButton(
			this.supportedFileExtensions.includes(fileFormat),
		);
	...
	async updateOuterEditButton(supported = false) {
		if (supported) {
			...
			this.outerEditButton.attr(
				"title",
				await I18n.translate("frontend.image_editor.edit_image"),
			);
		} else {
			this.outerEditButton.addClass("warning");
			this.outerEditButton.attr(
				"title",
				await I18n.translate("frontend.image_editor.unsupported_format"),
			);
		}
	}
	setup() {
		...
		TuiImageEditor().then((tuiImageEditor) => {
			this.editor = new tuiImageEditor(
				this.$reveal.find(".tui-image-editor").get(0),
				options,
			);
				...
				promise
					...
					.catch(async (data) => {
						...
						const errorMessage = await I18n.t(
							"frontend.image_editor.save_error",
						);
	...
	addCustomCropOptions(options) {
		...
		options.forEach((option) => {
			...
			customCropButton.classList.add(
		...
		for (let i = 0; i < cropList.children.length; i++) {
			...
			const customCropButtons = this.reveal.querySelectorAll(
				".preset-custom-option",
			);
			button.addEventListener("click", () => {
				customCropButtons.forEach((customCropButton) => {
					customCropButton.classList.remove("active");
				});
				if (button.classList.contains("preset-custom-option")) {
					const presetButtons = this.reveal.querySelectorAll(
						".preset:not(.preset-custom-option)",
					);
