...
class ObjectBrowser {
	static selector = '.object-browser[data-editable="true"]';
	static className = "object-browser";
	constructor(selector) {
		...
		this.removeAllButton = this.element
			.closest(".form-element")
			.querySelector("button.remove-all-objectbrowser");
	...
	setup() {
		...
		this.$element.on("dc:remove:data", this.#removeDataHandler.bind(this));
		...
		this.removeAllButton?.addEventListener("click", this.reset.bind(this));

		if (this.limitedByAttribute) this._addHandlersForLimitedBy();

		window.addEventListener("focus", this.highlightItems.bind(this));
	}
	#fieldForAttribute(attribute) {
		return this.element
			.closest("form")
			.querySelector(`.form-element[data-key*=${attribute}]`);
	}
	_addHandlersForLimitedBy() {
		this.limitedBy = $(this.#fieldForAttribute(this.limitedByAttribute));
	...
	initOverlayHandlers(_element) {
		...
		this.$overlay
			.find(".object-browser-filter .filter-sortable-checkbox-wrapper :input")
			.on("change", this.filterItems.bind(this));

		this.$overlay
			.find(".object-browser-filter .filter-sortable-select")
			.on("change", this.changeSortableDisabledStates.bind(this));
	...
	async importDataHandler(_event, data) {
		...
		this.$element.trigger("dc:import:data:complete");
	}
	#removeDataHandler(_event, data) {
		if (!data?.value) return;

		const query = data.value.map(
			(r) => `> .media-thumbs > .object-thumbs > li.item[data-id="${r}"]`,
		);
	...
	reset(_event) {
		this.$element
			.find('.media-thumbs li.item, .media-thumbs input[type="hidden"][value]')
			.each((_, element) => {
				this.removeThumbObject(element, true);
			});
	}
	setPreselected() {
		...
		this.chosen = $.map(preselected, (val, _i) => $(val).data("id"));
		this.$overlay
			.find(".chosen-items-container")
	...
	changeSortableDisabledStates(event) {
		const target = event.currentTarget;
		const sortableCheckbox = target.nextElementSibling.querySelector(
			'input[type="checkbox"]',
		);

		sortableCheckbox.disabled = !target.value;

		this.filterItems(event);
	}
	serializeFilter() {
		const filter = this.overlayFilterForm.serializeJSON();
		if (filter.s?.v) filter.s.v = this.transformSortParam(filter.s.v);

		return filter;
	}
	sortValueFor(sortMethod, attribute) {
		if (!attribute) return;

		const sortElement = this.#fieldForAttribute(attribute);
		const formData = getFormData(sortElement);
		let value = parseDataAttribute(
			Array.from(formData).find(([k, v]) => k.includes(attribute) && v)?.[1],
		);

		if (value && sortMethod === "proximity_geographic") {
			if (this.key.includes("start"))
				value = startFromCoordinates(value?.geometry?.coordinates);
			else if (this.key.includes("end"))
				value = endFromCoordinates(value?.geometry?.coordinates);
		}

		return value;
	}
	transformSortParam(sortParam) {
		if (!sortParam) return;

		const attribute =
			this.definition?.ui?.edit?.options?.sortable?.[sortParam.m]?.attribute;
		const sortValue = this.sortValueFor(sortParam.m, attribute);
