export default async function (id, duplicates) {
  return `
    ...
    <div class="dropdown-pane no-bullet bottom left-auto" id="${id}-duplicates-list" data-dropdown data-hover="true" data-hover-pane="true" aria-hidden="true">
      <h5>mögliche Duplikate</h5>
      <ul class="list-items duplicates-list no-bullet">
      ${duplicates
        .map(
          item =>
            `<li><a target="_blank" class="duplicate-link" href="/things/${item.id}"><img class="lazyload" data-src="${item.thumbnail_url}"></li>`
        )
        .join('')}
      </ul></div>
    `;
