// Original was written in CoffeeScript, now converted to JavaScript
...
is_end_of_tag = (char) => char === ">";

is_start_of_tag = (char) => char === "<";

is_whitespace = (char) => /^\s+$/.test(char);

is_tag = (token) => /^\s*<[^>]+>\s*$/.test(token);

isnt_tag = (token) => !is_tag(token);
...
op_map = {
	equal: (op, before_tokens, _after_tokens) =>
		before_tokens
			.slice(op.start_in_before, +op.end_in_before + 1 || 9e9)
			.join(""),
	insert: (op, _before_tokens, after_tokens) => {
		var val;
		val = after_tokens.slice(op.start_in_after, +op.end_in_after + 1 || 9e9);
		return wrap("ins", val);
	},
	delete: (op, before_tokens, _after_tokens) => {
		var val;
