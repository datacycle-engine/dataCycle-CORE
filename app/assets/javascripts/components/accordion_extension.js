class AccordionExtension {
	...
	setup() {
		$(document).on(
			"click",
			".accordion-close-all",
			this.closeChildren.bind(this),
		);
		$(document).on(
			"click",
			".accordion-open-all",
			this.openChildren.bind(this),
		);
		$(document).on(
			"click",
		...
		this.closeAccordionItems(
			...
			$(
				event.currentTarget.closest(
					".form-element.embedded_object, .embedded-viewer[data-accordion], .inner-container",
				),
			),
		);
	}
	...
	closeAccordionItems(event, container) {
		...
		$(container)
			.find(
				".embedded-viewer[data-accordion], .embedded-object[data-accordion], .attribute-group[data-accordion]",
			)
			.addBack(
				".embedded-viewer[data-accordion], .embedded-object[data-accordion], .attribute-group[data-accordion]",
			)
