class UndoRedoControl {
  ...
  _setupControls() {
    ...
    this.container.className = 'maplibregl-ctrl maplibregl-ctrl-group undo-control';
    ...
    I18n.translate('frontend.map.undo').then(text => (this.controlUndoButton.title = text));
    ...
    I18n.translate('frontend.map.redo').then(text => (this.controlRedoButton.title = text));
    ...
    MTK.event.addListener(this.mtkEditor, 'update', () => {
      if (!this._userChangedStep) {
        if (this._currentStep < this._stepHistory.length - 1)
          this._stepHistory = this._stepHistory.splice(0, this._currentStep + 1);
