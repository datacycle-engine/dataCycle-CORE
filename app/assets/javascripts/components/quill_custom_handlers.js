...
const QuillCustomHandlers = {
  ...
  async loadIconsWithTooltips(icons, toolbarConfig) {
    for (const key of toolbarConfig) {
      if (typeof key === 'string' && this.icons.hasOwnProperty(key)) await this.loadIconTranslation(icons, key);
    }
  },
  ...
  async inlineTranslator(_value) {
    ...
    const button = this.quill.root.closest('.editor-block').querySelector('.ql-inlineTranslator');
    const value = this.quill.root.innerHTML;

    DataCycle.disableElement(button, '<i class="fa fa-spinner fa-spin fa-fw"></i>');
    ...
    DataCycle.httpRequest({
      ...
      .then(async ({ text, detected_source_language: detectedSourceLocale }) => {
  ...
  async loadIconTranslation(icons, key) {
    ...
    icons[key] = this.icons[key].replace('data-dc-tooltip', `data-dc-tooltip="${text}"`);
  }
};
