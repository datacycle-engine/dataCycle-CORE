...
class CopyToClipboard {
  constructor(item) {
    this.item = item;
    this.item.classList.add('dcjs-copy-to-clipboard');
  ...
  copyValueToClipboard(event) {
    ...
    if (currentTarget.classList.contains('admin-clipboard'))
      currentTarget = currentTarget.closest('section.tabs-panel').querySelector('pre code');

    let text;
    if ('json' in currentTarget.dataset) text = currentTarget.dataset.json;
    else if ('value' in currentTarget.dataset) text = currentTarget.dataset.value;
    else text = currentTarget.innerText;
