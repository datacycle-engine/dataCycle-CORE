...
class CollectionOrderButton {
	constructor(button) {
		...
		this.newButtonObserver = new MutationObserver(
			this._observeHtmlContent.bind(this),
		);
		this.observerConfig = {
			attributes: false,
			characterData: false,
	...
	addDraggableHandle(item) {
		item
			...
			.insertAdjacentHTML("afterbegin", this.handleHtml);
	}
	enableSortable() {
		this.itemWrapper.classList.add("active");

		for (const item of this.sortableList.querySelectorAll("li.list-item"))
			this.addDraggableHandle(item);
	...
	disableSortable() {
		...
		for (const item of this.sortableList.querySelectorAll(".draggable-handle"))
			item.remove();
	...
	updateOrder(event) {
		...
		DataCycle.httpRequest(this.item.dataset.url, {
			...
			.catch(() => {
				I18n.t("collection.manual_order.error").then((text) =>
