...
class AssetDetailLoader {
  ...
  setup() {
    let loaderString = camelCase((this.validation && this.validation.class.split('::').pop() + 'Loader') || '');

    if (typeof this[loaderString] === 'function') this.loader = this[loaderString].bind(this);
  ...
  async videoLoader() {
    this.assetFile.prependHtml = this._fileThumbHtml('<i class="fa fa-video-camera" aria-hidden="true"></i>');
    ...
    try {
      ...
      this.assetFile.mediaHtml = await this.fileMediaHTML(
        ', ' +
          ...
          video.videoHeight +
          'px, ' +
          DurationHelpers.seconds_to_human_time(video.duration)
      );
      this.assetFile.validationOptions = {
  ...
  async audioLoader() {
    this.assetFile.prependHtml = this._fileThumbHtml('<i class="fa fa-file-audio-o" aria-hidden="true"></i>');
    this.assetFile._validateAndRender();
  }
  async pdfLoader() {
    this.assetFile.prependHtml = this._fileThumbHtml('<i class="fa fa-file-pdf-o" aria-hidden="true"></i>');
    this.assetFile._validateAndRender();
  }
  async textFileLoader() {
    this.assetFile.prependHtml = this._fileThumbHtml('<i class="fa fa-file-text-o" aria-hidden="true"></i>');
    this.assetFile._validateAndRender();
  }
  async defaultLoader() {
    this.assetFile.prependHtml = this._fileThumbHtml('<i class="fa fa-file-o" aria-hidden="true"></i>');
    this.assetFile._validateAndRender();
  }
}
