...
class ClassificationDragAndDrop {
	...
	updateOrder(event) {
		...
		this.sendUpdateRequest({
			...
			classificationTreeLabelId: element.closest("li.classification_tree_label")
				.id,
			previousAliasId: element.previousElementSibling?.dataset.id,
			newParentAliasId: element.parentElement.closest("li.direct")?.dataset.id,
	...
	sendUpdateRequest(data) {
		return DataCycle.httpRequest({
			...
			.catch(() =>
				I18n.t("classification_administration.move.error").then((text) =>
