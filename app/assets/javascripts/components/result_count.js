...
class ResultCount {
  ...
  setup() {
    ...
    this.countContainerObserver = new MutationObserver(() => {
      if (
        this.countContainer.innerHTML.trim() === "" &&
        !this.countContainer.classList.contains("loading")
      ) {
        this.loadCount();
      }
    });

    this.countContainerObserver.observe(this.countContainer, {
      childList: true,
      characterData: true,
      subtree: true,
    });
