...
class StoredSearchesFilter {
	constructor(inputField) {
		...
		this.listContainer = this.dropdownList.querySelector(
			".search-favorites-list",
		);
		this.changeObserver = new MutationObserver(
			this._checkForVisibility.bind(this),
		);
		this.currentRequest;
	...
	setup() {
		this.inputField.addEventListener(
			"keydown",
			this.preventEnterSubmit.bind(this),
		);
		this.inputField.addEventListener("change", this.preventDefault.bind(this));
		this.inputField.addEventListener(
			"input",
			debounce(this.filterStoredSearches.bind(this), 300),
		);
	...
	filterStoredSearches(event) {
		...
		const promise = DataCycle.httpRequest(this.url, {
			body: {
