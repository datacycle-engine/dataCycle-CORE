    ...
    this.quill.on(Quill.events.SELECTION_CHANGE, (range, oldRange, source) => {
      ...
      if (range.length === 0 && source === Quill.sources.USER) {
        ...
        if (link != null) {
          ...
          this.preview.setAttribute('data-dc-tooltip', 'dataCycle: ' + preview);
          this.show();
...
class ContentlinkBlot extends InlineBlot {
  static create(value) {
    ...
    node.setAttribute('data-dc-tooltip', 'dataCycle: ' + value);
    return node;
  }
  static formats(node) {
  ...
  format(name, value) {
    if (name !== this.statics.blotName || !value) {
      ...
      this.domNode.setAttribute('data-dc-tooltip', 'dataCycle: ' + value);
    }
  }
}
