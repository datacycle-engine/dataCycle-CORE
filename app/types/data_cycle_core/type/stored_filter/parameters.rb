    ...
    module StoredFilter
      class Parameters < ActiveRecord::ConnectionAdapters::PostgreSQL::OID::Jsonb
        ...
        def self.custom_param_transformation(hash, user)
          send("transform_#{hash['t']}", hash, user) if respond_to?("transform_#{hash['t']}")

          transform_placeholders(hash, user)
        end

        def self.transform_union(hash, user)
        ...
        def self.transform_with_classification_aliases_and_treename(hash, _user)
        ...
        def self.transform_external_source(hash, _user)
          hash['t'] = 'external_system'
          hash['n'] = hash['t'].capitalize
          hash['q'] = 'import'
        end

        def self.transform_external_system(hash, _user)
          hash['v'], hash['q'] = hash['v'].values_at('value', 'type') if hash['v'].is_a?(::Hash)
          hash['n'] = hash['t'].capitalize
          hash['q'] ||= 'import'
        end

        def self.transform_user_group_classifications(hash, user)
          hash['v'] = user&.id
        end

        def self.transform_creator(hash, _user)
          hash['t'] = 'user'
          hash['n'] = 'creator'
          hash['q'] = 'creator'
        end

        def self.transform_with_user_group_classifications_for_treename(hash, user)
        ...
        def self.transform_graph_filter(hash, _user)
