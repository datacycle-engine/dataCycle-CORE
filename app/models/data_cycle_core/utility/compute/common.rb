        ...
        class << self
          ...
          def get_values_from_hash(data_hash, key_path)
            ...
            if data_hash.is_a?(::Hash)
              ...
              data_hash.map { |v| get_values_from_hash({ key_path.first => v.property_value_for_set_datahash(key_path.first) }, key_path) }
            elsif data_hash.is_a?(::Array) && data_hash.first.to_s.uuid?
              DataCycleCore::Thing.where(id: data_hash).map { |v| get_values_from_hash({ key_path.first => v.property_value_for_set_datahash(key_path.first) }, key_path) }
