      ...
      module Video
        class << self
          def thumbnail_url(virtual_parameters:, content:, **_args)
            ...
            video = content.asset
            thumb_url = nil
            if DataCycleCore.experimental_features.dig('active_storage', 'enabled') && DataCycleCore.experimental_features.dig('active_storage', 'asset_types')&.include?(video.class.name) && video&.file&.attached?
              begin
