...
module DataCycleCore
  module Feature
    class MainFilter < Base
      attr_reader :advanced_filter_feature

      def initialize
        super
        @advanced_filter_feature = DataCycleCore::Feature::AdvancedFilter.new
      end
      ...
      def advanced_filters(user, config, selected_filters, key = :advanced, c = 'a', buttons = true)
        ...
        visible_filters = @advanced_filter_feature.available_visible_filters(user, config[:view_type], advanced_filter[:config])
      ...
      def available_user_advanced_filters(user, view)
        ...
        configuration.dig(:config, view, :filter).find { |f| f.key?('user_advanced') }&.values&.first&.each do |key, value|
          filters.concat(@advanced_filter_feature.try(key.to_sym, user, value) || @advanced_filter_feature.default(user, key.to_s, value) || [])
        end
