    ...
    class AdvancedFilter < Base
      ...
      def all_filters_by_locale(locale, filter = nil)
        ...
        @all_filters_by_locale ||= Hash.new do |h, key|
          h[key] = Rails.cache.fetch("#{cache_key_base}_all_filters_by_locale_#{key}") do
        ...
        filters = @all_filters_by_locale[locale.to_sym]
      ...
      def self.schedule_filter_exceptions
        Array.wrap(configuration.dig(:config, :schedule_exceptions))
      end

      def self.schedule_filter_exceptions_string(locale)
        schedule_filter_exceptions
          &.map { |e| I18n.t("schedule.filter_labels.#{e}", locale:) }
          &.join(', ')
      end

      def self.relation_filter_restrictions(type, name)
