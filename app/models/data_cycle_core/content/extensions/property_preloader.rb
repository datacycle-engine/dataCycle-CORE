    ...
    module Extensions
      module PropertyPreloader
        ...
        def preload_property(property_name, filter, overlay_flag)
          ...
          preload_overlays if (overlay_flag || embedded_property_names.include?(property_name)) && !@get_property_value&.key?(attibute_cache_key('overlay', nil, false))
        ...
        def add_preloaded_ccs(cc, a_key = :content_a_id, relation_key = :relation_a, b_key = :content_b_id)
          ...
          @_current_recursive_ccs[key_a]['_preload'] = @_current_recursive_ccs[key_a]['_preload'] || !cc.leaf
        end
        ...
        def preload_current_recursive_collection
          ...
          preloaded_ccs = @_current_collection.recursive_content_links(depth: 1).to_a
          ids_to_preload = []
          leaf_ids = []
          ...
          @_current_recursive_collection = self.class
            ...
            .tap { |rel| rel.send(:load_records, current_contents + values_to_preload) }

          ActiveRecord::Associations::Preloader.new.preload(@_current_recursive_collection.to_a, :translations)
        ...
        def preload_overlays
          overlay_templates = DataCycleCore::ThingTemplate.where(template_name: @_current_recursive_collection.map(&:overlay_template_name).compact).index_by(&:template_name)
          ...
          @_current_recursive_collection.each do |content|
            ...
            content.set_memoized_attribute(
              ...
              @_current_rc_with_leafs&.values_at(*@_current_recursive_ccs&.dig(content.id, content.overlay_name)),
              nil,
              false
            )
          end
              ...
              if content.translatable_property?(k)
                content.available_locales.each do |locale|
                  I18n.with_locale(locale) do
                    overlay_related_contents = overlay_related_contents&.filter { |t| t&.translated_locales&.include?(locale) }
        ...
        def preload_schedule_properties
          schedule_data = @_current_recursive_collection.schedules(preload: true).group_by(&:thing_id).transform_values! { |v| v.group_by(&:relation) }
        ...
        def preload_timeseries_properties
          timeseries_data = @_current_recursive_collection.timeseries(preload: true).group_by(&:thing_id).transform_values! { |v| v.group_by(&:property) }
