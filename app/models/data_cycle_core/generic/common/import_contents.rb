          ...
          I18n.with_locale(locale) do
            ...
            Array.wrap(options.dig(:import, :nested_contents)).each do |nested_contents_config|
              ...
              Array.wrap(nested_contents_items).each do |nested_data|
                ...
                process_single_content(utility_object, nested_contents_config[:template], transformation_method, nested_data, nested_content_config, options:)
              end
            end
            ...
            raw_data = raw_data.merge(options.dig(:import, :main_content, :data)) if options.dig(:import, :main_content, :data).present?
            process_single_content(utility_object, options.dig(:import, :main_content, :template), transformation_method, raw_data, main_content_config, options:)
          end
        end

        def self.process_single_content(utility_object, template_name, transformation_method, raw_data, config = {}, options:)
