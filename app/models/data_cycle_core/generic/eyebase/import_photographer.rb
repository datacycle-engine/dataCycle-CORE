        ...
        def self.load_contents(mongo_item, locale, source_filter)
          aggregation = mongo_item.where(
            source_filter.with_evaluated_values.merge({
              ...
              "dump.#{locale}.field_202.#cdata-section": { '$exists': true }
            })
          )
          aggregation = aggregation.project(
            'name' => "$dump.#{locale}.field_202.#cdata-section",
            "dump.#{locale}.name" => "$dump.#{locale}.field_202.#cdata-section"
          )
