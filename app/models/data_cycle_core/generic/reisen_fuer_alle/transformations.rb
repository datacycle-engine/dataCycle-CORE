    ...
    module ReisenFuerAlle
      module Transformations
        ...
        def self.to_rating(external_source_id)
          ...
          .>> t(:map_value, 'public_pdf', ->(s) { transform_pdfs(s) })
          .>> t(:map_value, 'short_report', ->(s) { transform_short_report(s) })
          .>> t(:unwrap, 'certificate_data')
          .>> t(:nest, 'certification_period', ['certified_from', 'certified_to'])
          .>> t(:map_value, 'certificate_type', ->(s) { transform_certificate_type(s) })
        ...
        def self.transform_pdfs(s)
          {
            'short_report' => s.dig('url_for_short_report', I18n.locale.to_s),
            'allergic' => s.dig('url_for_allergic', I18n.locale.to_s),
            'deaf' => s.dig('url_for_deaf', I18n.locale.to_s),
            'generations' => s.dig('url_for_generations', I18n.locale.to_s),
            'mental' => s.dig('url_for_mental', I18n.locale.to_s),
            'visual' => s.dig('url_for_visual', I18n.locale.to_s),
            'walking' => s.dig('url_for_walking', I18n.locale.to_s),
            'wheelchair' => s.dig('url_for_wheelchair', I18n.locale.to_s)
          }
        end

        def self.transform_short_report(s)
          {
            'deaf' => s.dig('deaf_and_partially_deaf_de'),
            'mental' => s.dig('mental_de'),
            'visual' => s.dig('visual_and_partially_visual_de'),
            'walking' => s.dig('wheelchair_and_walking_de')
          }
        end

        def self.transform_certificate_type(s)
          {
            'designation' => s.dig('label_de'),
            'key' => s.dig('key'),
            'icon' => s.dig('icon_url_de')
          }
        end
