...
module DataCycleCore
  module Generic
    class DownloadBase < Base
      ...
      def download_data(extract_id, extract_name, **options)
        options[:locales] ||= I18n.available_locales

        if options[:locales].size != 1
          options[:locales].each do |language|
            download_data(extract_id, extract_name, options.except(:locales).merge({ locales: [language] }))
          end
        else

          locale = options[:locales].first
          Mongoid.override_database("#{source_type.database_name}_#{external_source.id}")
          logging.preparing_phase("#{source_type.collection_name}_#{locale}")
          item_count = 0

          begin
            source_object.with(source_type) do |mongo_item|
              items = endpoint.send(options.dig(:download, :endpoint_method) || source_type.collection_name.to_s, lang: locale)

              max_string = ''
              max_string += (options[:max_count]).to_s if options[:max_count]
              logging.phase_started("#{source_type.collection_name}_#{locale}", max_string)
            ...
            logging.phase_finished("#{source_type.collection_name}_#{locale}", item_count)
          end
        end
      end
    end
