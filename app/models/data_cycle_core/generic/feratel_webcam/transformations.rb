    ...
    module FeratelWebcam
      module Transformations
        ...
        def self.to_ski_region(external_source_id)
          ...
          .>> t(:add_field, 'name', ->(s) { s.dig('c') || '__NO_NAME__' })
          .>> t(:add_links, 'lifts', DataCycleCore::Thing, external_source_id, ->(s) { DataCycleCore::Thing.where(external_key: "Feratel Webcams - Lift - #{s.dig('rid').downcase} - %", external_source_id: external_source_id)&.ids })
          .>> t(:add_links, 'slopes', DataCycleCore::Thing, external_source_id, ->(s) { DataCycleCore::Thing.where(external_key: "Feratel Webcams - Piste - #{s.dig('rid').downcase} - %", external_source_id: external_source_id)&.ids })
          .>> t(:strip_all)
        end
      end
    end
