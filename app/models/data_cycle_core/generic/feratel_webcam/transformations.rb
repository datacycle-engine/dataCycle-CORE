    ...
    module FeratelWebcam
      module Transformations
        ...
        def self.to_ski_region(external_source_id)
          ...
          .>> t(:add_field, 'name', ->(s) { s.dig('c') || '__NO_NAME__' })
          .>> t(:add_links, 'lifts_details', DataCycleCore::Thing, external_source_id, ->(s) { DataCycleCore::Thing.where('external_key ILIKE ? AND external_source_id = ?', "Feratel Webcams - Lift - #{s.dig('rid').downcase} - %", external_source_id)&.pluck(:external_key) })
          .>> t(:add_links, 'slopes_details', DataCycleCore::Thing, external_source_id, ->(s) { DataCycleCore::Thing.where('external_key ILIKE ? AND external_source_id = ?', "Feratel Webcams - Piste - #{s.dig('rid').downcase} - %", external_source_id)&.pluck(:external_key) })
