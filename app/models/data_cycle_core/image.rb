...
module DataCycleCore
  class Image < Asset
    ...
    def duplicate_candidates
      @duplicate_candidates ||= if duplicate_check&.dig('phash').blank?
                                  ...
                                  DataCycleCore::Image
                                    ...
                                    .where("thing_templates.schema -> 'features' -> 'duplicate_candidate' ->> 'module' = ?", 'BildPhash')
    ...
    def duplicate_candidates_with_score
      @duplicate_candidates_with_score ||= if duplicate_check&.dig('phash').blank?
                                             ...
                                             DataCycleCore::Image
                                               ...
                                               .map { |d| { thing_duplicate_id: d.thing_id, method: 'bild_phash', score: d.score } }
                                           end
    end
    ...
    def metadata_from_blob
      ...
      exif_data['ImColorSpace'] = IM_COLORSPACE_MAPPINGS[image.try(:interpretation).to_s.strip]
    ...
    def set_duplicate_hash
      ...
      update_column(:duplicate_check, { phash: Phash::Image.new(file.service.path_for(key)).try(:fingerprint) })
    end
  end
end
