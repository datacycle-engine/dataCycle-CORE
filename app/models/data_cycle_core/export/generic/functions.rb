      ...
      module Functions
        ...
        def self.enqueue_webhook(data, webhook, external_system)
          ...
          if delayed_job.nil? || data.webhook_as_of.present?
            ...
            delayed_job.update(run_at: [delayed_job.run_at, run_at].min, created_at: [delayed_job.created_at, run_at].min, updated_at: [delayed_job.updated_at, run_at].min, priority: [delayed_job.priority, priority].min, locked_by: nil, attemps: nil)
          end
        end
