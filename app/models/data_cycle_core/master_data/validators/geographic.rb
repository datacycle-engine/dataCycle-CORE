      ...
      class Geographic < BasicValidator
        ...
        def validate(data, template, _strict = false)
          if data.blank?
            ...
            convert_3d = nil
            begin
              RGeo::Geographic.spherical_factory(srid: 4326, wkt_parser: { support_wkt12: true }).parse_wkt(data)
            rescue RGeo::Error::ParseError
              convert_2d = I18n.t(:geo, scope: [:validation, :errors], data: data, template: template['label'], locale: DataCycleCore.ui_language)
            end
            begin
              RGeo::Geographic.spherical_factory(srid: 4326, has_z_coordinate: true, wkt_parser: { support_wkt12: true }).parse_wkt(data)
            rescue RGeo::Error::ParseError
