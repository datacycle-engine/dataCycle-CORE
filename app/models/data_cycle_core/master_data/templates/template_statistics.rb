      ...
      class TemplateStatistics
        ...
        def update_statistics
          DataCycleCore::ThingTemplate
            .where(updated_at: ...@start_time.utc.to_formatted_s(:long_usec))
        ...
        def render_statistics
          ...
          @outdated_templates.each do |value|
            puts "#{value[:name].to_s.ljust(40)} | #{value[:cache_valid_since].to_formatted_s(:long_usec).ljust(38)} | #{value[:count].to_s.rjust(12)} | #{value[:count_history].to_s.rjust(12)}"
          end
        end
