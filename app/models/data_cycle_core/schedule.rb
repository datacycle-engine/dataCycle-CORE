...
module DataCycleCore
  module ScheduleHandler
    ...
    def to_s
      "#{schedule_object} (#{dtstart&.to_formatted_s(:only_date)} - #{dtend&.to_formatted_s(:only_date)} // #{dtstart&.to_formatted_s(:only_time)} - #{(dtstart + (duration || 0))&.to_formatted_s(:only_time)})"
    end
    ...
    def to_opening_hours_specification_schema_org
...

      {
        ...
        'opens' => r_hash.dig(:start_time, :time)&.in_time_zone&.to_formatted_s(:only_time),
        'closes' => r_hash.dig(:end_time, :time)&.in_time_zone&.to_formatted_s(:only_time),
    ...
    def to_schedule_schema_org
      ...
      start_date = dtstart&.to_formatted_s(:only_date)
      start_time = dtstart&.to_formatted_s(:only_time)
      end_date = nil
      end_time = nil
      repeat_count = nil
      ...
      if schedule_object&.recurrence_rules&.first.present?
        ...
        end_date = dtend&.to_formatted_s(:only_date) if schedule_object.terminating?
        end_time = schedule_object&.end_time&.to_formatted_s(:only_time) if schedule_object.terminating?
        ...
        end_date = end_timestamp&.to_formatted_s(:only_date)
        end_time = end_timestamp&.to_formatted_s(:only_time)
      end
    ...
    def to_schedule_schema_org_api_v3
      ...
      start_date = dtstart&.beginning_of_day&.to_formatted_s(:long_msec)
      start_time = dtstart&.to_formatted_s(:only_time)
      end_date = nil
      end_time = nil
      repeat_count = nil
      ...
      if schedule_object&.recurrence_rules&.first.present?
        ...
        rule_hash = rule.to_hash
        end_date = schedule_object&.last&.in_time_zone&.+(duration.presence || 0)&.beginning_of_day&.to_formatted_s(:long_msec) if end_date.blank? && schedule_object.terminating?
        end_time = schedule_object&.last&.in_time_zone&.+(duration.presence || 0)&.to_formatted_s(:only_time) if end_time.blank? && schedule_object.terminating?
        repeat_count = rule&.occurrence_count
      ...
      schedule_object.all_occurrences.map do |occurrence|
        {
          ...
          'startDate' => occurrence.start_time&.to_formatted_s(:long_msec),
          'endDate' => occurrence.end_time&.to_formatted_s(:long_msec)
        }
      end
    end
    ...
    def to_event_dates
      ...
      if schedule_object.terminating?
        schedule_object.all_occurrences.to_a.map { |o| o.start_time.to_formatted_s(:long_msec) }
      else
        schedule_object.next_occurrences(10).to_a.map { |o| o.start_time.to_formatted_s(:long_msec) }
      end
    end
