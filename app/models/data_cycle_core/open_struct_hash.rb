    ...
    def to_h
      if table.blank?
        ...
        as_hash.each do |key, value|
          ...
          as_hash[key] = DataCycleCore::MasterData::DataConverter.convert_to_type(prop['type'], value, prop) if prop.present? && (Content::Content::PLAIN_PROPERTY_TYPES.include?(prop['type']) || ThingTemplateExtensions::PropertyTypes::GEO_PROPERTY_TYPES.include?(prop['type']))
        end
      end
      as_hash.compact
    end
