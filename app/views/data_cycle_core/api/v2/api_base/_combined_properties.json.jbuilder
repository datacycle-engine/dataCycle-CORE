...
combined.each do |combined_key, combined_value|
  if combined_value.dig('transformation', 'values') == 'concat'
    ...
    if content.translations.size > 1 && concated_keys_translateable?(combined_value.dig('items').map(&:first), content) && @include_parameters.include?('translations')
      ...
      combined_value['items'].each do |key, definition|
        content.translations.each do |translation|
          I18n.with_locale(translation.locale) do
            (concated_translated_value[translation.locale] ||= []) << concat_string((definition.dig('api', 'transformation', 'section_name') || key), content.send(key))
          end
        end
      ...
      concated_value = ''
      combined_value['items'].each do |key, definition|
        concated_value << concat_string((definition.dig('api', 'transformation', 'section_name') || key), content.try(key.to_sym))
      end
