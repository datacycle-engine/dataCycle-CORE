...
unless options[:header_type] == :overlay
  ...
  json.set! '@id', send(:"api_v2_#{content.model_name.element.tableize.singularize}_url", @api_subversion, content, @language == I18n.default_locale.to_s ? {} : { language: @language })
  json.set! 'identifier', content.id
  json.set! 'url', send(:"#{content.model_name.element.tableize.singularize}_url", content, @language == I18n.default_locale.to_s ? {} : { locale: @language })
...
if options[:header_type] == :full
  ...
  if @mode_parameters.include?('compact')
    classifications = content.concepts.distinct
  elsif content.respond_to?(:data_type)
    classifications = content.send(:data_type)
  end

  json.partial! 'classifications', concepts: classifications, key: 'classifications'
end
