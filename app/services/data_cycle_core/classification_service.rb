...
module DataCycleCore
  class ClassificationService
    ...
    def self.visible_classification_tree?(tree_label, scopes)
      tree_label_name = tree_label.is_a?(DataCycleCore::ConceptScheme) ? tree_label.name : tree_label
      tree_label_visibility = Rails.cache.fetch("#{tree_label_name}_visibilty", expires_in: 5.minutes) do
        tree_label = DataCycleCore::ConceptScheme.find_by(name: tree_label) unless tree_label.is_a?(DataCycleCore::ConceptScheme)
