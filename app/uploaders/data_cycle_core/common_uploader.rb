  ...
  class CommonUploader < CarrierWave::Uploader::Base
    ...
    def url
      ...
      I18n.with_locale(content.first_available_locale) do
        "#{asset_host}/assets/#{model.class.to_s.demodulize.underscore}/#{model.id}/#{version_name || 'original'}/#{(content.title.presence || File.basename(model.name.to_s, '.*')).underscore_blanks}.#{file&.extension}"
      end
    end
