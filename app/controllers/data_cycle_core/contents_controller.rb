  ...
  class ContentsController < ApplicationController
    ...
    def create
      ...
      I18n.with_locale(create_locale) do
        ...
        source = if source_params.present?
                   DataCycleCore::Thing.find_by(id: source_params[:source_id])
                 else
                   DataCycleCore::Thing.find_by(id: parent_params[:parent_id])
                 end
    ...
    def update
      ...
      I18n.with_locale(params[:locale] || @content.first_available_locale) do
        ...
        if @content.warnings.present?
          ...
          flash[:success] = I18n.t 'controllers.success.updated', data: @content.template_name, locale: helpers.active_ui_locale
        end
    ...
    def destroy
      ...
      I18n.with_locale(@content.first_available_locale(destroy_params[:locale])) do
        ...
        flash[:success] = @content.destroyed? ? I18n.t('controllers.success.destroyed', data: @content.template_name, locale: helpers.active_ui_locale) : I18n.t('controllers.success.destroyed_translation', data: @content.template_name, language: I18n.locale, locale: helpers.active_ui_locale)
    ...
    def compare
      ...
      redirect_back_or_to(root_path) && return if @diff_source.nil? || @content.nil?
      ...
      I18n.with_locale(@source_locale) do
        ...
        redirect_back_or_to(root_path, alert: helpers.tag.span(I18n.t('controllers.error.definition_mismatch', locale: helpers.active_ui_locale), title: "#{e.message.truncate(250)}\n\n#{e.backtrace.first(10).join("\n")}")) && return
      end
    end
    ...
    def history
      ...
      I18n.with_locale(@source_locale) do
        ...
        redirect_back_or_to(root_path, alert: helpers.tag.span(I18n.t('controllers.error.definition_mismatch', locale: helpers.active_ui_locale), title: "#{e.message.truncate(100)}\n\n#{e.backtrace.first(5).join("\n")}"), allow_other_host: false) && return
      end
    end
    ...
    def restore_history_version
      ...
      redirect_back_or_to(root_path, allow_other_host: false) && return if @history.nil? || @content.nil?

      I18n.with_locale(@history.first_available_locale) do
        if @history.restore_version(current_user: current_user)
          history_date_string = I18n.l(@history.updated_at, locale: helpers.active_ui_locale, format: :history) if @history.updated_at.present?
          flash[:success] = I18n.t('history.restore.version.restored', locale: helpers.active_ui_locale, date: history_date_string)
        else
          flash[:error] = @content.i18n_errors.map { |k, v| v.full_messages.map { |m| "#{k}: #{m}" } }.flatten
        end
    ...
    def validate
      @object = if validation_params[:id].present? && validation_params[:id].uuid?
                  DataCycleCore::Thing.find(validation_params[:id])
                elsif validation_params[:template].present?
                  DataCycleCore::Thing.new(template_name: validation_params[:template])
                else
                  raise ActionController::BadRequest, 'missing id or template parameter'
                end
    ...
    def load_more_duplicates
      ...
      @items = @content.duplicate_candidates.duplicates.includes(:duplicate_candidates).offset(DataCycleCore.linked_objects_page_size)
    ...
    def remove_locks
      ...
      flash[:success] = I18n.t 'controllers.success.removed_lock', locale: helpers.active_ui_locale
    ...
    def clear_cache
      ...
      @content = DataCycleCore::Thing.find(params[:id])
      @content.invalidate_self_and_embedded
    ...
    def trigger_webhooks
      content = DataCycleCore::Thing.find(trigger_webhooks_params[:id])
      webhook_action = trigger_webhooks_params[:webhook_action]
        ...
        format.turbo_stream do
          render turbo_stream: [
            ...
            turbo_stream.replace(
              "external-connections-#{content.id}",
              partial: 'data_cycle_core/contents/external_connections',
    ...
    def external_connection_params
      params.expect(external_system_sync: [:external_system_id, :external_key])
    end
    ...
    def content_params(template_name, params_hash = nil)
      params_hash = params[:thing] if params_hash.blank?
      DataCycleCore::DataHashService.new.permitted_content_params(template_name, params_hash)
    end
    ...
    def map_editor_params
      ...
      permitted = params.permit(:template_name, ids: [])
        .merge(params.slice(:stored_filter).permit!)
        .merge(params.slice(:filter).permit!)

      @map_editor_params = DataCycleCore::NormalizeService.normalize_parameters(permitted)
    end
  end
end
