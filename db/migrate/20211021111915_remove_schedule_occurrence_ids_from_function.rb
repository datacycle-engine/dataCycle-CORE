...
class RemoveScheduleOccurrenceIdsFromFunction < ActiveRecord::Migration[5.2]
  def up
    execute <<~SQL.squish
      DROP FUNCTION generate_schedule_occurences (uuid[]);

      DROP TABLE IF EXISTS schedule_occurrence_ids;

      CREATE OR REPLACE FUNCTION generate_schedule_occurences (schedule_ids uuid[])
        RETURNS SETOF uuid
        LANGUAGE PLPGSQL
        AS $$
      BEGIN
        ...
        WHERE
          ...
          AND NOT EXISTS (
            ...
            WHERE
              ...
              AND tstzrange(DATE_TRUNC('day', exdates.date), DATE_TRUNC('day', exdates.date) + INTERVAL '1 day') && tstzrange(occurences.occurence, occurences.occurence + occurences.duration))
        RETURNING
          id;
        RETURN;
      END;
