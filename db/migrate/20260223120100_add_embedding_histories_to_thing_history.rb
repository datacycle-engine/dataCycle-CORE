...
class AddEmbeddingHistoriesToThingHistory < ActiveRecord::Migration[7.1]
  def up
    execute <<~SQL.squish
      CREATE OR REPLACE FUNCTION public.to_embedding_history (content_id UUID, new_history_id UUID) RETURNS void LANGUAGE PLPGSQL AS $$
      DECLARE insert_query TEXT;

      BEGIN
      SELECT 'INSERT INTO embedding_histories (thing_history_id, ' || string_agg(column_name, ', ') || ') SELECT ''' || new_history_id || '''::UUID, ' || string_agg('t.' || column_name, ', ') || ' FROM embeddings t WHERE t.thing_id = ''' || content_id || '''::UUID;' INTO insert_query
      FROM information_schema.columns
      WHERE table_name = 'embedding_histories'
