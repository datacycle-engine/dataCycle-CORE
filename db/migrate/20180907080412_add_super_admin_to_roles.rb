...
class AddSuperAdminToRoles < ActiveRecord::Migration[5.1]
  def up
    ...
    DataCycleCore::User.where(email: super_admins).update(role_id: super_admin.id)
  end

  def down
    return unless DataCycleCore::Role.where(rank: 99).exists?

    super_admins = DataCycleCore::User.where(role_id: DataCycleCore::Role.find_by(rank: 99).id)
