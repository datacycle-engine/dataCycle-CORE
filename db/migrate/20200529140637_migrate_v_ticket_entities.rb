...
class MigrateVTicketEntities < ActiveRecord::Migration[5.2]
  def up
    ...
    vticket.imported_things.where(template_name: 'Event').each do |item|
      item.external_key = event_prefix + item.external_key
      item.save!
    end
    vticket.imported_things.where(template_name: 'Bild').each do |item|
      item.external_key = image_prefix + item.external_key
      item.save!
    end
    vticket.imported_things.where(template_name: 'Örtlichkeit').each do |item|
