...
class AddCleanupIndexesContent < ActiveRecord::Migration[5.1]
  def up
    remove_index :creative_works, name: 'index_creative_works_on_external_key'
    add_index :creative_works, [:external_source_id, :external_key], name: 'index_cw_on_external_source_id_and_external_key'
    remove_index :creative_works, name: 'index_creative_works_on_metadata_validation_name'

    add_index :event_translations, [:event_id, :locale], unique: true, name: 'by_et_ei_locale'
    add_index :events, :id, unique: true
    add_index :events, :external_source_id
    add_index :events, [:external_source_id, :external_key], name: 'index_e_on_external_source_id_and_external_key'
    execute <<-SQL
      CREATE INDEX index_events_on_content_type ON events ((schema ->> 'content_type'))
    SQL

    add_index :organization_translations, [:organization_id, :locale], unique: true, name: 'by_ot_ei_locale'
    add_index :organizations, :id, unique: true
    add_index :organizations, :external_source_id
    add_index :organizations, [:external_source_id, :external_key], name: 'index_o_on_external_source_id_and_external_key'
    execute <<-SQL
      CREATE INDEX index_organizations_on_content_type ON organizations ((schema ->> 'content_type'))
    SQL

    add_index :person_translations, [:person_id, :locale], unique: true, name: 'by_persont_pi_locale'
    add_index :persons, :id, unique: true
    add_index :persons, :external_source_id
    add_index :persons, [:external_source_id, :external_key], name: 'index_pers_on_external_source_id_and_external_key'
    execute <<-SQL
      CREATE INDEX index_persons_on_content_type ON persons ((schema ->> 'content_type'))
    SQL

    execute <<-SQL
      CREATE INDEX index_places_on_content_type ON places ((schema ->> 'content_type'))
    SQL
  end
