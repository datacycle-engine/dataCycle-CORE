...
class FixUpdateGeoPriosTriggerAgain < ActiveRecord::Migration[8.0]
  def up
    execute <<~SQL.squish
      CREATE OR REPLACE FUNCTION public.update_things_geo_priorities_trigger() RETURNS TRIGGER LANGUAGE 'plpgsql' AS $$ BEGIN PERFORM update_geo_priorities_by_thing_id(ARRAY [NEW.id]::UUID []);
