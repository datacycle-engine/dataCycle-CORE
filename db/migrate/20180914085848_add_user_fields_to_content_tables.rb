...
class AddUserFieldsToContentTables < ActiveRecord::Migration[5.1]
  def change
    ['creative_works', 'events', 'persons', 'places', 'organizations'].each do |content_table|
      ...
      add_column :"#{content_table.singularize}_histories", :created_by, :uuid
      add_column :"#{content_table.singularize}_histories", :updated_by, :uuid
      add_column :"#{content_table.singularize}_histories", :deleted_by, :uuid
    end
  end
end
