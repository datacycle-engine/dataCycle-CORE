...
class AddProperDictionaries < ActiveRecord::Migration[5.2]
  def up
    execute <<-SQL
      ...
      CREATE OR REPLACE FUNCTION update_with_dict(lang varchar) RETURNS varchar LANGUAGE PLPGSQL AS $$
      DECLARE
        dict varchar;
      BEGIN
        SELECT
      ...
      BEGIN
      	NEW.words := pg_catalog.to_tsvector(update_with_dict(NEW.locale)::regconfig, NEW.full_text::text);
        RETURN NEW;
      END;$$;
