...
class AddNewFunctionToGenerateCccFromClassifications < ActiveRecord::Migration[6.1]
  def up
    execute <<-SQL.squish
      ...
      CREATE OR REPLACE FUNCTION public.generate_ccc_relations_transitive_trigger_1() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM public.generate_ccc_from_ca_ids_transitive (array_agg(cccra.id))
      FROM (
      ...
      CREATE OR REPLACE FUNCTION public.delete_ccc_relations_transitive_trigger_2() RETURNS TRIGGER LANGUAGE plpgsql AS $$ BEGIN PERFORM public.generate_ccc_from_ca_ids_transitive (array_agg(cccra.id))
      FROM (
          SELECT DISTINCT ca.id
