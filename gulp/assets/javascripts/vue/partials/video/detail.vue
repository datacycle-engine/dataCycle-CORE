<template>
  <div v-if="item.id != undefined">
    ...
    <dl v-if="item.metadata != undefined">
      <dt class="block-flex" v-if="item.metadata.image != undefined"></dt>
      <dd class="block-flex" v-if="item.metadata.image != undefined">
        <img :src=" item.metadata.image ">
      </dd>
      <dt class="block-flex" v-if="item.metadata.url != undefined">Datei Url: </dt>
      <dd class="block-flex" v-if="item.metadata.url != undefined">
        <a :href="item.metadata.url " target="_blank ">{{ item.metadata.url }}</a>
      </dd>
      <dt v-if="item.metadata.file_format != undefined">Format: </dt>
      <dd v-if="item.metadata.file_format != undefined">{{ item.metadata.file_format }}</dd>
      ...
      <dt v-if="item.metadata.content_size != undefined">Dateigröße: </dt>
      <dd v-if="item.metadata.content_size != undefined">{{ formatSize(item.metadata.content_size) }}</dd>
      <dt v-if="item.metadata.director != undefined">Regie: </dt>
      <dd v-if="item.metadata.director != undefined">{{ item.metadata.director.given_name }} {{ item.metadata.director.family_name }}</dd>
      <dt v-if="item.metadata.contributor != undefined">Kamera: </dt>
      <dd v-if="item.metadata.contributor != undefined">{{ item.metadata.contributor.given_name }} {{ item.metadata.contributor.family_name }}</dd>
      <dt class="block-flex" v-if="item.metadata.license != undefined">Lizenz: </dt>
      ...
      <dd class="block-flex" v-if="item.metadata.restrictions != undefined && item.metadata.restrictions != '' && item.metadata.restrictions != 'Keine Einschränkung;'">
        <i class="fa fa-exclamation-triangle" aria-hidden="true"></i> {{ item.metadata.restrictions }}</dd>
