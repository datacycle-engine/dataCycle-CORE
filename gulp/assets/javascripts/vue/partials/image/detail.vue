<template>
  <div v-if="item.id != undefined">
    ...
    <dl v-if="item.metadata != undefined">
      ...
      <dd class="block-flex" v-if="item.metadata.restrictions != undefined && item.metadata.restrictions != '' && item.metadata.restrictions != 'Keine Einschränkung;'"><i v-if="item.metadata.restrictions.indexOf('gesperrt') !== -1" class="fa fa-exclamation-triangle" aria-hidden="true"></i> {{ item.metadata.restrictions }}</dd>
