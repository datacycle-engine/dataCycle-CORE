...
module.exports.initialize = function() {
  ...
  $(document).on('contents-added', (event, data) => {
    if (
      ...
      $(event.target).data('id') != undefined &&
      $(event.target).data('label') != undefined
    ) {
  ...
  $(document).on('click', '.flex-box .copy', event => {
    ...
    if ($(event.currentTarget).data('copy-attribute') == 'data-id')
      value = $(event.currentTarget)
        .parents('[data-editor]')
        .data('id');
    else if ($(event.currentTarget).data('copy-attribute') == 'html')
      value = $(event.currentTarget)
        .parents('[data-editor]')
        .find('.detail-content')
        .html();
    else if ($(event.currentTarget).data('copy-attribute') == 'single-data-id')
      value = $(event.currentTarget)
        .parents('.copy-single')
        .data('id');
  ...
  function add_buttons(element, label, value, copy_attr, single = false) {
    if (
      single &&
      $(element).hasClass('copy-single') &&
      $('.flex-box .edit-content [data-label="' + label + '"]').length > 0 &&
      value.length > 0
    ) {
      if ($(element).children('.buttons').length)
        $(element)
          .children('.buttons')
          .append(
            '<a class="button-prime small copy copy-single-button" data-copy-attribute="single-data-id" title="Übernehmen"><i class="fa fa-arrow-right" aria-hidden="true"></i></a>'
          );
      else
        $(element).append(
          '<a class="button-prime small copy copy-single-button" data-copy-attribute="single-data-id" title="Übernehmen"><i class="fa fa-arrow-right" aria-hidden="true"></i></a>'
      ...
      $('.flex-box .edit-content [data-label="' + label + '"]').length > 0 &&
      value.length > 0 &&
      $(element).children('.buttons').length > 0
    ) {
      $(element)
        .children('.buttons')
        .append(
          '<a class="button-prime small copy" data-copy-attribute="' +
            copy_attr +
      ...
      $(element).append(
        '<div class="buttons"><a class="button-prime small copy" data-copy-attribute="' +
          copy_attr +
          '" title="Übernehmen"><i class="fa fa-arrow-right" aria-hidden="true"></i></a></div>'
      );
    }
  }
  ...
  function init(container) {
    $(container)
      .children('div[data-editor=object_browser]')
      .each((idx, elem) => {
    ...
    $(container)
      .children('div[data-editor=embedded_object]')
      .each((idx, elem) => {
        add_buttons(elem, $(elem).data('label'), $(elem).data('id') || [], 'data-id');
      });

    $(container)
      .children('div[data-editor=string], div[data-editor=text_editor]')
      .each((idx, elem) => {
        add_buttons(
          elem,
          $(elem).data('label'),
