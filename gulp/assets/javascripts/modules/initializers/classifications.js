...
module.exports.initialize = function() {
  let load_sub_classifications = function(location_array, index) {
    if (location_array != undefined && index < location_array.length) {
      ...
      if (!link.length) {
        ...
        let more_link = $(
          "#" + prev_id + " > .children > .load-more-link > .inner-item > a"
        ).last();
  ...
  if ($("#classification-administration").length) {
          ...
          ajax: {
            ...
            processResults: function(data) {
              ...
              return {
                results: data
                  .map(value => {
                    if (value.classification_id != undefined)
    ...
    $("#classification-administration").on("click", ".discard", function(
      event
    ) {
      $(this)
        .parents("form")
  ...
  if ($("#classification-tree-label-list").length) {
    $("#classification-tree-label-list").on("ajax:beforeSend", "a", function(
      event,
      xhr,
      options
    ) {
