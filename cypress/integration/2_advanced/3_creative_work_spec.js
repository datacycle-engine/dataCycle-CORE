describe('CreativeWork', function () {
  ['Artikel', 'Angebot', 'SocialMediaPosting'].forEach(option => {
    context(option, function () {
      ...
      it('create', function () {
        ...
        cy.get('#new-object .option[data-open="' + option_name + '"]').then(function ($elem) {
          ...
          cy.get('.flash.callout').should('be.visible').should('have.class', 'success').find('.close-button').click().should('not.be.visible');
