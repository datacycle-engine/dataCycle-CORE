describe('Embedded Object', function () {
  ...
  it('use Objectbrowser inside Quote', function () {
    ...
    cy.get('.new-content-reveal:visible').should('be.visible').find('#person_datahash_given_name').should('be.visible').type(person.given_name)
    cy.get('.new-content-reveal:visible').should('be.visible').find('#person_datahash_family_name').should('be.visible').type(person.family_name + '{enter}')
